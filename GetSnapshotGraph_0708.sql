...
-- Description:	
-- Call SP:			dbo.GetSnapshotGraph
-- =============================================
CREATE PROCEDURE [dbo].[GetSnapshotGraph_0708]
    (
      @AppUserId BIGINT ,
      @ActivityId BIGINT ,
...
AS
    BEGIN
        ...
        DECLARE @listStr NVARCHAR(MAX),     @QuestionSearch NVARCHAR(4000);

		IF @FilterOn <> '' AND @FilterOn IS NOT NULL
            SET @QuestionSearch = '';
        ...
        IF ( @IsOut = 1 )
			...
			PRINT 1
                SELECT  @ToatlWaitage = dbo.PICalculationForGraph_Snapshot(@ActivityId,
                                                              ...
                                                              @IsOut, @UserId,
                                                              @EstablishmentId,
                                                              0, @AnswerMaster,@FormStatus,@ReadUnread,@isAction,@isTransfer,@table);
                                ...
                                INSERT  INTO @Result
                                        ...
                                        FROM    ( SELECT    Am.ReportId ,
                                                  ...
                                                  FROM      dbo.View_AnswerMaster
                                                            ...
                                                            AS A ON Am.ReportId = A.AnswerMasterId AND ISNULL(A.IsNA,0) = 0
															INNER JOIN @QuestionSearchTable QS ON (QS.ReportId = Am.ReportId OR QS.ReportId = 0)
                                                            INNER JOIN dbo.Questions Q ON A.QuestionId = Q.Id
                                                              AND Q.DisplayInGraphs = 1
                                                            INNER JOIN ( SELECT
                                    ...
                                    BEGIN
                                        ...
                                        INSERT  INTO @Result
                                                ...
                                                FROM    ( SELECT
                                                        ...
                                                        --      OR @UserId = '0'
                                                          WHERE
                                                              Am.QuestionnaireId = @QuestionnaireId
                                                              AND Isnull(A.RepetitiveGroupId,0) != 0
                                                              AND ISNULL(Am.IsDisabled,
                                                              0) = 0
                                                              AND ( IsResolved = @AnsStatus
							...
							PRINT 3
                                INSERT  INTO @Result
                                        ...
                                        FROM    dbo.View_SeenClientAnswerMaster
                                                ...
                                                AS A ON Am.ReportId = A.SeenClientAnswerMasterId AND ISNULL(A.IsNA,0) = 0
												INNER JOIN @QuestionSearchTable QS ON (QS.ReportId = Am.ReportId OR QS.ReportId = 0)
                                ...
                                INSERT  INTO @Result
                                        ...
                                        FROM    ( SELECT    Q.Id ,
                                                            ( AVG(A.Weight)
                                                              * 100 )
                                                            / AVG(CASE Q.MaxWeight WHEN 0 THEN 1 ELSE Q.MaxWeight END) AS Detail ,
                                                            SUM(CASE ISNULL(A.Detail,
                                                              '')
                                                              WHEN '' THEN 0
                                                              ELSE 1
                                    ...
                                    BEGIN
                                        ...
                                        INSERT  INTO @Result
                                                ...
                                                FROM    ( SELECT
                                                              ...
                                                              END AS [PI]
                                                          FROM
                                                              dbo.View_SeenClientAnswerMaster
                                                              AS Am
                                                              INNER JOIN dbo.SeenClientAnswers
                                                              AS A ON Am.ReportId = A.SeenClientAnswerMasterId AND ISNULL(A.IsNA,0) = 0
															  INNER JOIN @QuestionSearchTable QS ON (QS.ReportId = Am.ReportId OR QS.ReportId = 0)
				...
				--				PRINT 'OUt6.1'
                                ...
                                INSERT  INTO @FinalResult
                                        ...
                                        SELECT  @FromDate AS StartDate ,
                                                ...
                                                ISNULL(ROUND(SUM(BenchMarkScore),
                                                             0), 0) AS PerformanceBenchmark ,
        ...
        SELECT  
			    ...
			    CASE ISNULL(SUM(Counts), 0) WHEN 0 THEN -1 ELSE ISNULL(SUM(Score), 0.00) end AS Score ,
				ISNULL(SUM(PerformanceBanchMark), 0.00) AS PerformanceBanchMark ,
                ISNULL(YourScore, 0.00) AS YourScore ,
                ISNULL(CASE @AnswerMaster WHEN 0 THEN BenchMarkScore ELSE 0.00 END, 0.00) AS BenchMarkScore ,
                ISNULL(YourScore - (CASE @AnswerMaster WHEN 0 THEN BenchMarkScore ELSE 0 END), 0.00) AS PerformanceBenchMarkScore ,
        ...
        FROM    @FinalResult INNER JOIN @Tbl ON [@Tbl].ShortName = [@FinalResult].ShortName
        GROUP BY [@FinalResult].ShortName ,
                 ...
                 EndDate ,
                 LastUpdatedDate,
				 [@Tbl].POSITION ORDER BY [@Tbl].POSITION;
    END;
