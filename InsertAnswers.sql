...
-- =============================================
CREATE PROCEDURE dbo.InsertAnswers
...
BEGIN
    ...
    BEGIN CATCH
        ...
        (ERROR_LINE(),
         ...
         GETUTCDATE(),
         ISNULL(@AppUserId, 0),
         @AnswerMasterId + ',' + @QuestionId + ',' + @QuestionTypeId + ',' + @Detail + ',' + @AppUserId + ','
         + @RepeatCount + ',' + @RepetitiveGroupId + ',' + @RepetitiveGroupName + ',' + @CreatedOn,
         GETUTCDATE(),
         ISNULL(@AppUserId, 0)
        );
    END CATCH;
