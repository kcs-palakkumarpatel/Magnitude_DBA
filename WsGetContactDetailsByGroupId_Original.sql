-- EXEC WsGetContactDetailsByGroupId 201,610
CREATE PROCEDURE [dbo].[WsGetContactDetailsByGroupId_Original]
    @GroupId BIGINT ,
    @AppUserId BIGINT
AS
    BEGIN
            ...
            BEGIN
                INSERT  INTO @SeenclinetContact
                        ...
                        FROM    ( SELECT TOP 50
                                            ...
                                            SAM.SeenClientId ,
											  ( CASE WHEN SAM.IsSubmittedForGroup = 1 THEN 
                                                    CG.LastUsedOn ELSE
                                                   CM.LastUsedOn
                                              END ) AS LastUsedOn,
                    ...
                    BEGIN
                        INSERT  INTO @FinalResult
                                ...
                                FROM    dbo.ContactGroup AS CG
                                        INNER JOIN ContactRole AS c ON c.GroupId = @GroupId
                                        INNER JOIN @RoleDetails AS crd ON crd.ContactRoleId = c.Id
                                                              AND ( crd.AppEstablishmentUserId = CG.CreatedBy
                                                              OR CG.CreatedBy = 0
                                                              )
                                        INNER JOIN dbo.AppUserContactRole AS ac ON ac.ContactRoleId = c.Id
                                                              AND ac.AppUserId = @AppUserId
...
INSERT INTO @Result
        ...
        ELSE
			...
			ELSE
             dbo.ConcateString('ContactSummary',
			dbo.DefaultContact.ContactId) end AS ContactGropName ,IsGroup, 
