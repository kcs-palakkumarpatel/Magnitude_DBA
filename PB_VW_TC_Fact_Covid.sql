...
CREATE view [dbo].[PB_VW_TC_Fact_Covid] as
...
and CD.contactQuestionId=3049
)+' '+(SELECT TOP 1 cd.Detail FROM ContactDetails CD WHERE   CD.ContactMasterId=AM.ContactMasterId and CD.IsDeleted = 0 and CD.Detail<>'' 
and CD.contactQuestionId=3050
) as ResponsibleUser,SH.StatusDateTime, es.StatusName  as StatusName
from dbo.[Group] G
inner join EstablishmentGroup EG on G.id=EG.groupid and G.Id=503 and EG.Id =5977
...
select EstablishmentName,0 as ReferenceNo,[Name] +' '+
[Surname] as ResponsibleUser,null as StatusDateTime,''as StatusName,
ResponseDate,ResponseReferenceNo,
[Location],
[Fever],
[Cough],
[Sore Throat],
...
,Q.shortname as Question ,Am.Longitude,Am.Latitude
from dbo.[Group] G
inner join EstablishmentGroup EG on G.id=EG.groupid and G.Id=503 and EG.Id =6011
inner join Establishment E on  E.EstablishmentGroupId=EG.Id --and E.id in (27195,27196,27200,27199)
inner join AnswerMaster AM on AM.EstablishmentId=E.id ANd (AM.IsDeleted=0 or AM.IsDeleted=null)
inner join [Answers] A on A.AnswerMasterId=AM.id
inner join Questions Q on Q.id=A.QuestionId --and  Q.id in (34009,34011,34012,34013,34014,34015,34016)
