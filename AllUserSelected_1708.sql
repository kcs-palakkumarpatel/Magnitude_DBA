...
-- select dbo.AllUserSelected_1708(1615,'0',2011)

-- =============================================
CREATE FUNCTION dbo.AllUserSelected_1708
    (
      @AppUserId BIGINT ,
...
BEGIN
		...
		IF EXISTS ( SELECT 1
                    ...
                    AND ( IsAreaManager = 0 OR AppUserId = @AppUserId)
                    AND IsActive = 1)
		BEGIN
			SET @Count = 1
		END
		...
		BEGIN
			IF EXISTS (SELECT 1
					...
					INNER JOIN dbo.Establishment AS E ON E.Id = AppManagerUserRights.EstablishmentId AND E.EstablishmentGroupId = @ActivityId
                ...
                ELSE
                    BEGIN
                        ...
                        INNER JOIN dbo.AppUser AS U ON AppUser.AppUserId = U.Id 
							AND ( U.IsAreaManager = 0 OR U.Id = @AppUserId)
                        WHERE   E.IsDeleted = 0
							AND UE.IsDeleted = 0
