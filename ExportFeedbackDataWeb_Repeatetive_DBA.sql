...
-- =============================================  
CREATE PROCEDURE dbo.ExportFeedbackDataWeb_Repeatetive_DBA
(
...
BEGIN
	...
	SET @TEMP = 'SELECT  A.ReportId  FROM    dbo.View_AllAnswerMaster AS A    INNER JOIN (SELECT * FROM dbo.Split(''' + @EstablishmentId + '''
	, '','')) AS E ON A.EstablishmentId = E.Data OR '''
          + @EstablishmentId + ''' = ''0'' INNER JOIN (SELECT * FROM dbo.Split('''+'1,2'+'''
    ...
    SET @SqlselectCount
        = N'SELECT  A.ReportId  FROM    dbo.View_AllAnswerMaster AS A    INNER JOIN (SELECT * FROM dbo.Split(''' + @EstablishmentId + ''', '','')) AS E ON A.EstablishmentId = E.Data OR '''
          + @EstablishmentId + ''' = ''0''  
    ...
    BEGIN
        ...
        (
            ...
            SELECT Q.Position,
                   ...
                   --        CQ.ShortName  
                   --    ELSE  
                   --('(' + ISNULL(ANS.RepetitiveGroupName, 0) + CAST(ISNULL(ANS.RepeatCount, 0) AS VARCHAR(20)) + ')'  
                   -- + Q.ShortName  
                   --)  
                   --END AS [Question],  
                   ANS.Question,
