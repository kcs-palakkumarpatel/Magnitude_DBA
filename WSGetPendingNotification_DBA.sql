CREATE PROCEDURE dbo.WSGetPendingNotification_DBA
...
SET NOCOUNT ON 
    ...
    BEGIN
        ...
        BEGIN
		  ...
		  SET @FilterDate += ' AND P.CreatedOn BETWEEN DATEADD(MINUTE,-(CASE WHEN ( P.ModuleId IN (2,5,7,11)) THEN AM.TimeOffSet
          WHEN ( P.ModuleId IN (3,6,8,12)) 
		  THEN SAM.TimeOffSet ELSE AM.TimeOffSet END),''' + CONVERT(VARCHAR(19), @FromDate, 120) + ''') AND DATEADD(MINUTE,-(CASE WHEN ( P.ModuleId IN (2,5,7,11)) THEN AM.TimeOffSet
          WHEN ( P.ModuleId IN (3,6,8,12)) 
		  THEN SAM.TimeOffSet ELSE AM.TimeOffSet END),'''
                               + CONVERT(VARCHAR(19),@ToDate, 120) + ''') ';
        END;
        /* Date Filter */
        /* Read Filter */
        IF (@IsRead = '1')
...
BEGIN CATCH
	INSERT INTO dbo.ErrorLog
        ...
        VALUES
        (ERROR_LINE(),
         'dbo.WSGetPendingNotification_DBA',
         N'DATABASE',
         ERROR_MESSAGE(),
         ...
         GETUTCDATE(),
         ISNULL(@AppUserId,0),
         @AppUserId+','+@Sortby+','+@TextSearch+','+@ActivityId+','+@UserId+','+@Type+','+@FormStatus+','+@IsRead+','+@FromDate+','+@ToDate+','+@Page+','+@Rows+','+@Flag+','+@UserFilter+','+@isOld,
         GETUTCDATE(),
         ISNULL(@AppUserId,0)
        );
END CATCH
