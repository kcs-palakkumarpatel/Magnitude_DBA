...
=============================================  
*/
CREATE PROCEDURE dbo.SearchAppUser  
    @Rows INT ,  
    @Page INT ,  
    @Search NVARCHAR(500) ,  
    @Sort NVARCHAR(50) ,  
...
BEGIN  
	...
	IF @AdminRole = @UserRole  
		BEGIN  
			...
			GroupName,[Password],
			'' AS Establishment,
			--ISNULL([TU].[EstablishmentNameStr], '') AS Establishment,
			IsActive 
			FROM (
				SELECT  RowNum,ISNULL(Total, 0) AS Total,Id,Name,Email,Mobile,IsAreaManager,ISNULL(SupplierId, 0) AS SupplierId,UserName,GroupId,
				GroupName,[Password],T.IsActive  
				FROM (
					SELECT dbo.[AppUser].[Id] AS Id,dbo.[AppUser].[Name] +' '+ dbo.[AppUser].[LastName] AS Name,dbo.[AppUser].[Email] AS Email,dbo.[AppUser].[Mobile] AS Mobile,
					...
					AND (ISNULL(dbo.[AppUser].[Name], '') LIKE '%' + @Search + '%'  
						OR ISNULL(dbo.[AppUser].[LastName], '') LIKE '%' + @Search + '%'  
					...
					GROUP BY  dbo.[AppUser].[Id],dbo.[AppUser].[Name],dbo.[AppUser].[LastName],dbo.[AppUser].[Email],dbo.[AppUser].[Mobile],dbo.[AppUser].[IsAreaManager],
	...
	ELSE  
		BEGIN  
			...
			FROM (
				...
				T.IsActive  
				FROM (
					SELECT DISTINCT dbo.[AppUser].[Id] AS Id,dbo.[AppUser].[Name] +' '+ dbo.[AppUser].[LastName] AS Name,dbo.[AppUser].[Email] AS Email,dbo.[AppUser].[Mobile] AS Mobile,
					dbo.[AppUser].[IsAreaManager] AS IsAreaManager,dbo.[AppUser].[SupplierId] AS SupplierId,dbo.[AppUser].[UserName] AS UserName,
					dbo.[AppUser].[GroupId] AS GroupId,dbo.[Group].GroupName,[Password],'' AS Establishment,dbo.[AppUser].IsActive AS IsActive,  
					COUNT(1) OVER (PARTITION BY 1) AS Total,
					ROW_NUMBER() OVER (ORDER BY CASE WHEN @Sort = 'Id Asc' THEN dbo.[AppUser].[Id] END ASC, 
