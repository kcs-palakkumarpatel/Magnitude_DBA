...
-- =============================================
CREATE PROCEDURE dbo.RegisterFeedBackEmailSMS_07032019Notifiactionchnageafter
...
BEGIN
    ...
    BEGIN
        ...
        WHERE AU.Id = @AppUserId
              AND AUE.NotificationStatus = 1 AND au.IsActive = 1;
    END;
    ELSE
    BEGIN
        ...
        WHERE UE.EstablishmentId = @EstablishmentId
              AND UE.NotificationStatus = 1 AND U.IsActive = 1;
    END;
    ...
    BEGIN
        ...
        FROM dbo.AnswerMaster SA
            INNER JOIN dbo.AppUser A
                ON SA.CreatedBy = A.Id AND A.IsActive = 1
        ...
        FROM dbo.AppUserEstablishment AS AUE
            ...
            INNER JOIN dbo.AppUser AS U
                ON U.Id = T.AppUserId AND U.IsActive = 1
        ...
        FROM dbo.AnswerMaster SA
            INNER JOIN dbo.AppUser A
                ON SA.CreatedBy = A.Id
                   AND SA.Id = @AnswerMasterId AND A.IsActive = 1
        UNION
        SELECT DISTINCT
            2,
            @Notification,
        ...
        FROM dbo.AppUserEstablishment AS AUE
            ...
            INNER JOIN dbo.AppUser AS U
                ON U.Id = ISNULL(amu.UserId, AUE.AppUserId)
                   AND AUE.EstablishmentId = SCA.EstablishmentId AND U.IsActive = 1
        WHERE AUE.IsDeleted = 0
              AND AUE.NotificationStatus = 1
              AND U.IsDeleted = 0
              AND U.IsActive = 1
