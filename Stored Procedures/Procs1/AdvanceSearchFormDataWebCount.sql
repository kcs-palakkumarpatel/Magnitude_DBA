...
--=============================================
CREATE PROCEDURE dbo.AdvanceSearchFormDataWebCount
...
BEGIN
    ...
    IF (@QuestionSearch IS NOT NULL AND @FormType = 'In')
    BEGIN
        SET @SqlselectCount
            = N'SELECT  TOP 1
				IIF(''' + @ActivityType
              + ''' = ''Customer'', SUM(1) OVER ( ), SUM(CASE WHEN ( A.IsOut = 0 ) THEN 1 ELSE 0 END) OVER ( )) AS TotalRecord,
    ...
    BEGIN
        SET @SqlselectCount
            = N'SELECT  TOP 1
				IIF(''' + @ActivityType
              + ''' = ''Customer'', SUM(1) OVER ( ), SUM(CASE WHEN ( A.IsOut = 1 ) THEN 1 ELSE 0 END) OVER ( )) AS TotalRecord,
				SUM(CASE WHEN A.SmileType = ''Positive'' THEN 1 ELSE 0 END) OVER ( ) AS PositiveCount ,
                SUM(CASE WHEN A.SmileType = ''Negative'' THEN 1 ELSE 0 END) OVER ( ) AS NegativeCount ,
                SUM(CASE WHEN A.SmileType = ''Neutral'' THEN 1 ELSE 0 END) OVER ( ) AS PassiveCount ,				SUM(CASE WHEN A.AnswerStatus = ''Unresolved'' THEN 1 ELSE 0 END) OVER ( ) AS UnresolvedCount ,
				SUM(CASE WHEN A.AnswerStatus = ''Resolved'' THEN 1 ELSE 0 END) OVER ( ) AS ResolvedCount ,
    ...
    BEGIN
        ...
        BEGIN
            --SET @Filter += ' AND A.AnswerStatus = ''Resolved''';
            SET @FilterCount += ' AND A.AnswerStatus = ''Resolved'' AND A.IsOut = 1';
        END;
        IF (@FormStatus = 'Unresolved')
        BEGIN
            --SET @Filter += ' AND A.AnswerStatus = ''Unresolved''';
    ...
    BEGIN
        ...
        BEGIN
            SET @FilterCount += ' AND A.SmileType = ''' + @SmileyTypesSortby + '''AND A.IsOut = 1';
        END;
        ELSE
        BEGIN
            SET @FilterCount += ' AND A.SmileType = ''' + @SmileyTypesSortby + ''' AND A.IsOut = 1';
    ...
    EXEC ('select A.SeenClientAnswerMasterId FROM  dbo.View_AllAnswerMaster AS A '
    --+ @SqlSelect12 + @SqlSelect2 + @Filter
    + @SqlSelect12 + @SqlSelect2 + @FilterCount + ' Group By A.SeenClientAnswerMasterId'
         );
    ...
    DECLARE @Temp1 TABLE ([SeenClientAnswerMasterId] [BIGINT] NOT NULL);
    ...
    BEGIN
        ...
        EXEC ('select A.SeenClientAnswerMasterId FROM  dbo.View_AllAnswerMaster AS A ' + @SqlSelect12 + @SqlSelect2 + @FilterCount + ' Group By A.SeenClientAnswerMasterId');
        --SET @Temp2 =('select A.SeenClientAnswerMasterId FROM  dbo.View_AllAnswerMaster AS A ' + @SqlSelect12 + @SqlSelect2 + @FilterCount + ' Group By A.SeenClientAnswerMasterId')
        --SELECT @Temp2
        DECLARE @var VARCHAR(MAX);
        SET @var =
        (
    ...
    IF (@isFlag = 'true')
    BEGIN
        SET @FilterCount += ' AND F.IsFlag = 1 AND A.IsOut = 1';
    END;
    ...
    BEGIN
        ...
        SET @FilterCount += ' AND dbo.GetMObilink(A.ReportId, ' + CONVERT(VARCHAR(10), @AppuserId)
                            + ', CASE WHEN A.ContactGroupId != 0 THEN 1 ELSE 0 END) != ''''';
    END;
