...
-- =============================================
CREATE PROCEDURE [dbo].[GetAllocatedTaskListCountUserWise_BackUp]
...
BEGIN;
    ...
    SELECT ISNULL(COUNT(1), 0),
           ...
           (SELECT AP.Name
               ...
               WHERE CM.Id = SCA.ContactMasterId
                     AND CM.IsDeleted = 0
                     AND CD.QuestionTypeId IN ( 10 )
                     AND CD.Detail <> '') AS Name
	...
	AND (AUT.Userid = SCA.AppUserId OR @IsManager = 0 )
			  AND (SCA.ContactMasterId IN
              (
                  SELECT CM.Id
                  FROM dbo.ContactMaster AS CM
                      INNER JOIN dbo.ContactDetails AS CD
    ...
    GROUP BY SI.Id,
             AUT.ActivityId,
             SCA.AppUserId,SCA.ContactMasterId;
    ...
    SELECT UTT.AppUserName,
          ...
          ISNULL((SELECT ISNULL(UTC.Totaltask, 0) FROM #UserTotalCompleted UTC WHERE UTT.AppUserId = UTC.AppUserId),0) AS TotalCompleted,
           (
               SELECT TOP 1
                      CASE
                          WHEN ImageName <> '' THEN
    ...
    FROM #UserTotaltask UTT
       WHERE ((UTT.AppUserName LIKE '%' + @SearchText +'%') OR @SearchText = '' ) ORDER BY UTT.AppUserName DESC OFFSET ((@Page - 1) * @Rows) ROWS FETCH NEXT @Rows ROWS ONLY;
END;
