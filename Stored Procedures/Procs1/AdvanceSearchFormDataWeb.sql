...
--=============================================
CREATE PROCEDURE dbo.AdvanceSearchFormDataWeb
...
AS
    BEGIN
		...
		--IF EXISTS (SELECT 1 FROM #ActionFilterId)
		--BEGIN
         ...
         IF ( @TemplateId != '' )
            BEGIN
                ...
                INSERT  INTO #ActionFilterId
                        SELECT  ISNULL(AnswerMasterId,
                                       SeenClientAnswerMasterId)
                        FROM    dbo.CloseLoopAction
                        WHERE   IsDeleted = 0
                                AND ( Conversation LIKE '%' + ( SELECT
                                                              @ActionFilter
                                                              ) + '%' );
	...
	FROM    dbo.View_AllAnswerMaster (NOLOCK) AS A ';
        ...
        SET @SqlSelect2 += CHAR(13)
			...
			+ CHAR(13)
            + ' 	LEFT OUTER JOIN dbo.FlagMaster AS F ON F.ReportId = A.ReportId AND F.Type IN (1,2) AND F.AppUserId = ''' + CONVERT(VARCHAR(10) , @AppuserId) + '''';
    SET @Filter += CHAR(13)
