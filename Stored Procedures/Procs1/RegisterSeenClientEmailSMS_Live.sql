...
-- Call SP:RegisterSeenClientEmailSMS '<ClsAnswers><row><lgAnswerMasterId>1010742</lgAnswerMasterId><SeenClientAnswerChildId /><lgSeenClientId>2739</lgSeenClientId><lgEstablishmentId>33505</lgEstablishmentId><lgAppUserId>18215</lgAppUserId><EncryptedId>YwAgVaP0YqI1</EncryptedId><lstReoccurring></lstReoccurring><Resend>false</Resend></row></ClsAnswers>'
-- =============================================
CREATE PROCEDURE dbo.RegisterSeenClientEmailSMS_Live @RegisterSeenClientEmailSMS XML
AS
BEGIN
       ...
       SELECT @EmailReceiver = COALESCE(@EmailReceiver, '') + Email + ',',
        ...
        BEGIN
		 print 'email'
            ...
            IF @Email <> ''
               AND @Email IS NOT NULL AND LOWER(@Email) NOT IN (Select LOWER(EmailId) from BlackListEmail where IsDeleted = 0)
            BEGIN
			   print 'email1'
                ...
                BEGIN
                    ...
                    FROM dbo.Split(@RRules, ',')
					WHERE Data <> ''
					AND Data NOT IN (Select EmailId from BlackListEmail where IsDeleted = 0);
        ...
        BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    WHERE EmailReceiver <> ''
					AND EmailReceiver NOT IN (Select EmailId from BlackListEmail where IsDeleted = 0)
