...
-- =============================================  
CREATE PROCEDURE [dbo].[CreateBiData_15Sept2021]
(
  @ActivityId BIGINT,
  @AppUserID BIGINT,
  @IsOut BIT
...
END
    ...
    SELECT @ActionFilter
        = ' AND A.ReportId EXISTS (SELECT ISNULL(AnswerMasterId,SeenClientAnswerMasterId)   
...
END
 ...
 --                       + @UserId + ''' = ''0'' OR A.UserId = 0 ';
    ...
    IF (@isAction = 'true')
    BEGIN
        SET @Filter += ' AND A.IsActioned = 1 AND A.ReportId EXISTS (SELECT ISNULL(AnswerMasterId,SeenClientAnswerMasterId) FROM dbo.CloseLoopAction WHERE Conversation LIKE ''%'
                       + @ActionSearch + '%'')';
        SET @FilterCount += ' AND A.IsActioned = 1 AND A.ReportId EXISTS (SELECT ISNULL(AnswerMasterId,SeenClientAnswerMasterId) FROM dbo.CloseLoopAction WHERE Conversation LIKE ''%'
                            + @ActionSearch + '%'')';
    END;
            ...
            BEGIN
                ...
                SET @FilterCount += 'AND (' + (CASE @IsOut
                                                   ...
                                                   ELSE
                                                       ' OSeenAns'
                                               END
                                              ) + @QuestionId + '.Data EXISTS ( SELECT Data FROM dbo.Split('''
                                    + @SearchText + ''', '','')) )';
            END;
            ELSE
            BEGIN
