...
-- =============================================
CREATE PROCEDURE [dbo].[APIRegisterSeenClientEmailSMSSCB]
...
AS
   ...
   --         BEGIN
        ...
        DECLARE @SendSMS BIT ,
			...
			--IF ((SELECT COUNT(*) FROM dbo.AppUser WHERE id IN (SELECT AppUserId FROM dbo.AppUserEstablishment WHERE EstablishmentId = @EstablishmentId) AND IsAreaManager = 1) > 0)
   --         BEGIN
   ...
   --         BEGIN
                ...
                FROM    dbo.AppUser AS U
                        INNER JOIN dbo.AppUserEstablishment AS UE ON UE.AppUserId = U.Id
                                                              AND UE.IsDeleted = 0
            ...
            --END;
                ...
                FROM    dbo.AppUser
                WHERE   Id = @AppUserId AND IsDeleted = 0 AND IsActive = 1;
                ...
                SELECT TOP 1
                        ...
                        @DelayTime = CASE WHEN Eg.AllowToChangeDelayTime = 1
                                          THEN ISNULL(UE.DelayTime,
                                                      Eg.DelayTime)
                                          ELSE Eg.DelayTime
                                     END ,
											...
											CASE E.SeenClientEscalationTime
											  ...
											  CASE WHEN (DATEADD(HOUR,
                                                           ...
                                                           DATEADD(MINUTE,
                                                              ...
                                                              E.SeenClientSchedulerTime)
                                                              - E.TimeOffSet,
                                                              CAST(CAST(GETUTCDATE() AS DATE) AS DATETIME)))) > GETUTCDATE() THEN 
															  (DATEADD(HOUR,
                                                           DATEPART(HOUR,
                                                              E.SeenClientSchedulerTime),
                                                           DATEADD(MINUTE,
                                                           ...
                                                           DATEADD(MINUTE,
                                                              ...
                                                              E.SeenClientSchedulerTime)
                                                              - E.TimeOffSet,
                                                              CAST(CAST(GETUTCDATE() AS DATE) AS DATETIME))))) END
                                              WHEN 0
           ...
           --                                 END ,
                ...
                FROM    dbo.GetSeenClientAutoSMSEmailNotificationText(@SeenClientAnswerMasterId,
                                                              @EncryptedId,
                ...
                IF @SendSMS = 1
                    ...
                    BEGIN
                        ...
                        WHERE   SeenClientAnswerMasterId = @SeenClientAnswerMasterId
                                AND ( ISNULL(SeenClientAnswerChildId, 0) = ISNULL(@SeenClientAnswerChildId,
                                                              0) )
                                AND IsDeleted = 0
                ...
                IF @SendEmail = 1
                    ...
                    BEGIN	
                        ...
                        WHERE   SeenClientAnswerMasterId = @SeenClientAnswerMasterId
                                AND ( ISNULL(SeenClientAnswerChildId, 0) = ISNULL(@SeenClientAnswerChildId,
                                                              0) )
															  AND QuestionId = @SeenClientQuestionID
                                AND IsDeleted = 0
                            ...
                            BEGIN
                                INSERT  INTO dbo.PendingNotification
                                        ...
                                        FROM    dbo.AppUserEstablishment AS UE
                                                INNER JOIN dbo.AppUser AS U ON UE.AppUserId = U.Id AND U.IsDeleted = 0 AND U.IsActive = 1
                                                INNER JOIN dbo.UserTokenDetails
                                                AS T ON UE.AppUserId = T.AppUserId
                                                INNER JOIN dbo.Establishment
                                                AS E ON UE.EstablishmentId = E.Id
                            ...
                            BEGIN
                                INSERT  INTO dbo.PendingNotification
                                        ...
                                        SELECT  3 ,
												...
												@AppUserId
                                        FROM    dbo.AppUserEstablishment AS UE
                                                INNER JOIN dbo.AppUser AS U ON UE.AppUserId = U.Id AND U.IsActive = 1 AND U.IsDeleted = 0
                                                INNER JOIN dbo.UserTokenDetails
                                                AS T ON UE.AppUserId = T.AppUserId
                                                INNER JOIN dbo.Establishment
                                                AS E ON UE.EstablishmentId = E.Id
                                                INNER JOIN dbo.AppManagerUserRights
                                                AS AM ON AM.ManagerUserId = U.Id
                ...
                IF ( @SendEmail = 1
                    ...
                    END;
		IF (ISNULL((SELECT COUNT(1) FROM dbo.PendingEmail WHERE RefId = @SeenClientAnswerMasterId AND CAST(ScheduleDateTime AS DATE) = CAST(GETUTCDATE() AS DATE)
		AND EmailId IN (SELECT Data FROM dbo.Split(@EmailReceiver, ','))),0) = 0)
		BEGIN
                                    ...
                                    BEGIN
                                        INSERT  INTO dbo.PendingNotification
                                                ...
                                                FROM    dbo.AppUserEstablishment
                                                        ...
                                                        AS U ON UE.AppUserId = U.Id AND U.IsActive = 1 AND U.IsDeleted = 0
                                                        INNER JOIN dbo.Establishment
                                                        AS E ON UE.EstablishmentId = E.Id
                                                WHERE   UE.IsDeleted = 0
                                                        AND E.IsDeleted = 0
                                    ...
                                    BEGIN
                                        ...
                                        INSERT  INTO dbo.PendingNotification
                                                ...
                                                FROM    dbo.AppUserEstablishment
                                                        ...
                                                        AS E ON UE.EstablishmentId = E.Id
                                                        INNER JOIN dbo.AppManagerUserRights
                                                        AS AM ON AM.EstablishmentId = E.Id
