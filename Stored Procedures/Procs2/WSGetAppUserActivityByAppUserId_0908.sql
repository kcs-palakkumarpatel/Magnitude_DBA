...
--	Description:	
--	Call SP: dbo.WSGetAppUserActivityByAppUserId_0908 1615, ''
--	=============================================
CREATE PROCEDURE dbo.WSGetAppUserActivityByAppUserId_0908
    @AppUserId BIGINT ,
...
SELECT DISTINCT 
				  CASE ISNULL(T.DisplaySequence,0) WHEN 0 THEN 99999 ELSE T.DisplaySequence END DisplaySequence, T.ActivityName,	
				  T.ActivityId ,
                  --T.ActivityName ,
                  T.ActivityType ,
                  T.IsNotificationOn ,
...
(SELECT dbo.GetBadgeCountINOUT(@AppUserId, T.ActivityId,T.LastDays,1)) AS OUTCount,
(SELECT dbo.GetBadgeCountINOUT(@AppUserId, T.ActivityId,T.LastDays,0)) AS InCount,
0 AS OUTCount,
0 AS InCount,
T.LastDays,
(SELECT dbo.GetCountForResponse(@AppUserId,T.ActivityId)) AS ResponseCount
--0 AS ResponseCount
 FROM (
SELECT DISTINCT EG.Id AS ActivityId ,
		...
		(SELECT ISNULL(MAX(ActivitySequence),EG.DisplaySequence) FROM dbo.AppUserEstablishment WHERE AppUserId = @AppUserId AND EstablishmentId IN (SELECT id FROM dbo.Establishment WHERE EstablishmentGroupId = EG.Id AND IsDeleted = 0)) AS DisplaySequence,
...
WHERE   EG.IsDeleted = 0
 ...
 --                 T.IsTellUsSubmitted,
	--			  T.LastDays
	--		ORDER BY CASE ISNULL(T.DisplaySequence,0) WHEN 0 THEN 99999 ELSE T.DisplaySequence END, T.ActivityName ASC;
