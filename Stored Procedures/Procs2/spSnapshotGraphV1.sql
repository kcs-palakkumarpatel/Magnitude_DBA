...
-- Description:	
-- Call SP:		dbo.spSnapshotGraphV1 13959, 2951, '31 Oct 2017', 3, '1699,1700,1750', 0,'',1699,0
-- =============================================
CREATE PROCEDURE [dbo].[spSnapshotGraphV1]
...
AS
    BEGIN
                            ...
                            BEGIN
                                ...
                                INSERT  INTO @FinalResult
                                        ...
                                        SELECT  @FromDate AS StartDate ,
                                                ...
                                                [@Result].TotalEntry AS TotalEntry
                                        FROM    @Tbl
        ...
        SELECT  StartDate ,
                EndDate ,
                LastUpdatedDate ,
                CASE when LEN(ShortName) <= 11 then ShortName ELSE SUBSTRING(ShortName,1,11) + '...' END AS ShortName,
                SUM(ISNULL(Counts, 0)) AS Counts ,
                SUM(Score) AS Score ,
                SUM(PerformanceBanchMark) AS PerformanceBanchMark ,
                YourScore ,
                CASE @AnswerMaster
                  WHEN 0 THEN BenchMarkScore
                  ELSE 0
                END AS BenchMarkScore ,
                YourScore - ( CASE @AnswerMaster
                                WHEN 0 THEN BenchMarkScore
                                ELSE 0
                              END ) AS PerformanceBenchMarkScore ,
                --PerformanceBenchMarkScore ,
                MinRank ,
                MaxRank ,
                GroupType ,
                SUM(TotalEntry) AS TotalEntry
        FROM    @FinalResult
		GROUP BY 
		StartDate ,
                EndDate ,
