...
*/
CREATE PROCEDURE dbo.WsGetSeenClientOutDataForMobiForm_101120
...
BEGIN
	...
	SELECT @Id = CASE IsSubmittedForGroup WHEN 1 THEN ContactGroupId ELSE ContactMasterId END,
		...
		@ActivityId = (SELECT EstablishmentGroupId FROM dbo.Establishment WHERE Id = SAM.EstablishmentId) 
	...
	IF ((SELECT COUNT(1) FROM dbo.SeenClientQuestions
              WHERE QuestionTypeId IN ( 1, 5, 6, 7, 18, 21 )
              AND SeenClientId = ( SELECT SeenClientId FROM dbo.EstablishmentGroup WHERE  Id = @ActivityId)
                                AND [Required] = 1 AND IsDeleted = 0) > 0 )
	BEGIN
		SET @PIDispaly = '1';
	END;
	...
	FROM ( SELECT DISTINCT R.ReportId ,
                ...
                IsDisplayInDetail ,
                A.QuestionTypeId ,
                CASE SQ.QuestionTypeId WHEN 8 THEN dbo.ChangeDateFormat(Detail, 'dd/MMM/yyyy')
                              WHEN 9 THEN dbo.ChangeDateFormat(Detail, 'hh:mm AM/PM')
                              WHEN 22 THEN dbo.ChangeDateFormat(Detail,'dd/MMM/yyyy hh:mm AM/PM')
                              WHEN 1 THEN dbo.GetOptionNameByQuestionId(A.QuestionId,A.Detail, 1)
                              WHEN 23 THEN SQ.ImagePath ELSE ISNULL(A.Detail, '') END AS Detail ,
                CASE SQ.QuestionTypeId WHEN 23 THEN @GraphicImagePath ELSE @Url END AS Url ,
				CASE SQ.QuestionTypeId WHEN 23 THEN @GraphicImagePath ELSE @ThumbnilUrl END AS ThumbnilUrl,
                R.IsTransferred ,
                R.AnswerStatus ,
                R.IsActioned ,
                SQ.Position ,
			...
			FROM (SELECT TOP 1
					...
					AND ((ContactMasterId = @Id AND @IsGroup = 0 ) OR ( ContactGroupId = @Id AND @IsGroup = 1 ))
