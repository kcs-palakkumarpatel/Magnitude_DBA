...
-- Call SP    :	SearchDraftedFormsDataWeb 5093,5193,'','2020-10-12 14:28:15','2020-10-13 14:28:17'
-- =============================================
CREATE PROCEDURE [dbo].[SearchDraftedFormsDataWeb]
    (
      @AppuserId BIGINT = 0 ,
...
AS
    BEGIN
		...
		ISNULL(ES.EstablishmentGroupId, 0) AS ActivityId,
                ...
                dbo.ChangeDateFormat(DATEADD(MINUTE, ES.TimeOffSet, SAM.CreatedOn),'dd/MMM/yy hh:mm') AS CaptureDate ,
                ...
                dbo.fn_SummaryQuestionsList('SeenClientAnswers', SAM.Id) AS DisplayText
        ...
        WHERE   SAM.IsDeleted = 1
				...
				--AND SAM.CreatedOn BETWEEN @FromDate AND @ToDate
                AND ( ES.EstablishmentName LIKE '%' + @Search + '%'
                      OR AU.Name LIKE '%' + @Search + '%'
					  OR dbo.ChangeDateFormat(DATEADD(MINUTE, ES.TimeOffSet, SAM.CreatedOn),'dd/MMM/yy hh:mm') LIKE '%' + @Search + '%'
                      OR REPLACE(dbo.fn_SummaryQuestionsList('SeenClientAnswers', SAM.Id), '\n', '') LIKE '%' + @Search + '%'
                    )
