...
CREATE PROCEDURE dbo.SendPendingSMSListByModule_BP @ModuleId BIGINT
AS
BEGIN
    ...
    BEGIN
        ...
        FROM dbo.PendingSMS
            INNER JOIN dbo.SeenClientAnswerMaster SCAM
                ON dbo.PendingSMS.RefId = SCAM.Id
    ...
    BEGIN
        ...
        FROM dbo.PendingSMS
            INNER JOIN dbo.AnswerMaster AM
                ON dbo.PendingSMS.RefId = AM.Id
            INNER JOIN dbo.Establishment E
                ON AM.EstablishmentId = E.Id
                   AND E.GroupId NOT IN ( @IndianGroup )
        WHERE (IsSent = 0)
              AND (ScheduleDateTime <= GETUTCDATE())
              AND (dbo.PendingSMS.IsDeleted = 0)
              AND ModuleId = @ModuleId
    ...
    BEGIN
        ...
        SELECT TOP 1000
            ...
            dbo.PendingSMS.SMSType AS SMSType
        FROM dbo.PendingSMS
            INNER JOIN dbo.SeenClientAnswerMaster AM
                ON dbo.PendingSMS.RefId = AM.Id
            INNER JOIN dbo.Establishment E
                ON AM.EstablishmentId = E.Id
                   AND E.GroupId NOT IN ( @IndianGroup )
    ...
    SET @COUNT =
    (
        SELECT COUNT(*) FROM @PendingSmsTable
    );
    SET @COUNT = @COUNT + 1;
