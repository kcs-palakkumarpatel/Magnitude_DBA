...
-- =============================================
CREATE PROCEDURE [dbo].[SeenclientandFeedbackIspositiveUpdate_111921]
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            END;
        END;
        IF (ISNULL(@EscalationValue, 0) = 0)
        BEGIN
            SET @EscalationValue = -1;
        END;
        IF (@IsAutoResolved = 1 AND @EscalationValue < ROUND(@PI, 0))
        BEGIN
            DECLARE @QEscalationValue DECIMAL(18, 2) = 0,
                    @QQuestionnaireId BIGINT = 0,
                    @QAnswerEscalationValue DECIMAL(18, 2) = 0;

            SELECT @QEscalationValue = Q.EscalationValue
            FROM dbo.EstablishmentGroup Eg
                INNER JOIN dbo.Establishment E
                    ON E.EstablishmentGroupId = Eg.Id
                INNER JOIN dbo.SeenClientAnswerMaster SCA
                    ON SCA.Id = @AnswerMasterId
                       AND SCA.EstablishmentId = E.Id
                INNER JOIN dbo.Questionnaire Q
                    ON Q.Id = Eg.QuestionnaireId;

            SELECT @QAnswerEscalationValue = MAX(EscalationValue)
            FROM dbo.Questions
            WHERE QuestionnaireId = @QQuestionnaireId
                  AND ISNULL(IsDeleted, 0) = 0;

            IF (
                   ISNULL(@QEscalationValue, 0) = 0
                   AND ISNULL(@QAnswerEscalationValue, 0) = 0
               )
            BEGIN
                UPDATE dbo.SeenClientAnswerMaster
        ...
        BEGIN
            ...
            END;
            DECLARE @IsResolvedTrue BIT = 0;
            IF (@IsAutoResolved = 1)
            BEGIN
                IF (ISNULL(@EscalationValue, 0) = 0 OR @EscalationValue = -1)
                BEGIN
                    SET @EscalationValue = -1;
                    IF (ISNULL(@PI, 0) <= 0)
                    BEGIN
                        SET @IsResolvedTrue = 1;
                    END;
                END;
                ELSE
                BEGIN
                    IF (@EscalationValue < ROUND(ISNULL(@PI, 0), 0))
                    BEGIN
                        SET @IsResolvedTrue = 1;
                    END;
                END;
