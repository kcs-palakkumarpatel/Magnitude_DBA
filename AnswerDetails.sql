...
-- Description:		
-- Calls :SELECT dbo.AnswerDetails ('SeenClientAnswers',696207)
-- =============================================    
CREATE FUNCTION [dbo].[AnswerDetails]
    (
      @Table NVARCHAR(50) ,
      @FilterValue BIGINT    
    )
						...
						DISTINCT
                                ...
                                CASE Q.QuestionTypeId
                                  ...
                                  THEN ( CASE WHEN A.Detail IS NULL
                                                   OR A.Detail = '' OR A.Detail ='NaN-aN-aN aN:aN:aN'
                                              THEN ISNULL(A.Detail, '')
                                              ELSE  dbo.ChangeDateFormat(CAST(A.Detail AS DATETIME), 'dd/MMM/yyyy')
                                         END )
                                  WHEN 9
                                  THEN ( CASE WHEN A.Detail IS NULL
                                                   OR A.Detail = '' OR A.Detail ='NaN-aN-aN aN:aN:aN'
                                              THEN ISNULL(A.Detail, '')
                                              ELSE  dbo.ChangeDateFormat(A.Detail, 'hh:mm AM/PM')
                                         END )
                                  WHEN 22
                                  THEN ( CASE WHEN A.Detail IS NULL
                                                   OR A.Detail = '' OR A.Detail ='NaN-aN-aN aN:aN:aN'
                                              THEN ISNULL(A.Detail, '')
                                              ELSE  dbo.ChangeDateFormat(A.Detail, 'dd/MMM/yyyy hh:mm AM/PM')
                                         END )
                                  WHEN 1
