...
--=============================================
CREATE PROCEDURE dbo.AdvanceSearchFormData_Backup02Aug
...
AS
	...
	/*	---------------------------PI Filter------------------------------ */
        ...
        SET @SqlselectCount = N'SELECT TOP 1 Sum(CASE WHEN SmileType = ''Positive'' THEN 1 ELSE 0 END) OVER() AS PositiveCount ,Sum(CASE WHEN SmileType = ''Negative'' THEN 1 ELSE 0 END)  OVER() AS NegativeCount ,Sum(CASE WHEN SmileType = ''Neutral'' THEN 1 ELSE 0 END)  OVER() AS PassiveCount ,
    ...
    END;
        ...
        IF ( @IsResponse = 'true' )
            ...
            END;
                ...
                + '  A.ReportId ,A.EstablishmentId ,A.EstablishmentName ,A.UserId ,A.UserName ,A.SenderCellNo ,A.IsOutStanding ,A.AnswerStatus ,A.TimeOffSet ,A.CreatedOn ,
									A.UpdatedOn,A.EI ,A.[PI]  ,A.SmileType ,A.QuestionnaireType ,A.FormType ,A.IsOut ,A.QuestionnaireId ,A.ReadBy ,A.ContactMasterId ,A.ContactGroupId,A.Latitude ,A.Longitude ,
                                    A.IsTransferred ,A.TransferToUser ,A.TransferFromUser ,A.SeenClientAnswerMasterId ,A.ActivityId ,A.IsActioned ,A.TransferByUserId ,A.TransferFromUserId,A.isdisabled,A.CreatedUserId) AS R';
