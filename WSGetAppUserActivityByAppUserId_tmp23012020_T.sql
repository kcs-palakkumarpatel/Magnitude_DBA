...
--	=============================================
CREATE PROCEDURE [dbo].[WSGetAppUserActivityByAppUserId_tmp23012020_T]
    @AppUserId BIGINT,
...
--BEGIN
	...
	INNER JOIN dbo.AppUserEstablishment Aue WITH(NOLOCK)  ON est.Id = EstablishmentId AND Aue.IsDeleted = 0  
	AND EST.IsDeleted=0
	where Aue.AppUserId = @AppUserId 
...
--BEGIN
        ...
        WHERE   Id = @AppUserId;
			...
			IF EXISTS ( SELECT 1
					...
					INNER JOIN dbo.Establishment AS E ON E.Id = AppUserEstablishment.EstablishmentId AND E.EstablishmentGroupId = AppUser.Id
                    UNION
					SELECT 1
                    FROM  dbo.AppUserEstablishment
