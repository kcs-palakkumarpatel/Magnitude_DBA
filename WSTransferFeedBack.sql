...
-- Description:	
-- Call SP:		WSTransferFeedBack 132158, 4938, 4936, 1, 22539
-- =============================================
CREATE PROCEDURE dbo.WSTransferFeedBack
...
BEGIN
    DECLARE @AMId BIGINT,
            @NewAppUserName VARCHAR(100),
            @FromAppUserName VARCHAR(100),
            @Notification NVARCHAR(500);

    SELECT @NewAppUserName = [Name]
    FROM dbo.AppUser
    WHERE Id = @NewAppUserId;
    SELECT @FromAppUserName = [Name]
    FROM dbo.AppUser
    WHERE Id = @AppUserId;
    ...
    BEGIN
        ...
        WHERE (
                  SeenClientAnswerMasterId = @AnswerMasterId
                  AND IsDeleted = 0
              );
        ...
        BEGIN
            ...
            SELECT @CurrentStatusHistoryId = StatusHistoryId,
                   @Offset = TimeOffSet,
                   @SHLat = Latitude,
                   @ShLong = Longitude
            FROM SeenClientAnswerMaster
            ...
            SELECT @CurrentStatusIconImageId = ES.StatusIconImageId
            FROM StatusHistory SH
                LEFT JOIN EstablishmentStatus ES
        ...
        (
            ...
            FROM dbo.Establishment
            WHERE Id = @EstablishmentId AND ISNULL(InitiatorAsRespondent,0) = 1
        )
        BEGIN
            DECLARE @NewAppUserContactMasterId BIGINT,
                    @SCACId BIGINT,
                    @isExistingContactMasterNewAppUser BIT = 0,
                    @CurSeenClientId BIGINT,
                    @GroupId BIGINT,
                    @oldSenderCellNo NVARCHAR(100);
            SET @NewAppUserContactMasterId =
            (
                SELECT dbo.fn_IsContactMasterExistsByAppUserId(@NewAppUserId)
            );

            PRINT 'APP User Contact Master ID';
            ...
            BEGIN
                ...
                DECLARE @IsContactGroup BIT,
                        @CurContactGroupId BIGINT,
                        @CurContactMasterId BIGINT,
                        @ContactMasterIDs NVARCHAR(MAX),
                        @ContactGroupName NVARCHAR(MAX),
                        @ExistingGroupId BIGINT = 0,
                        @curGroupId BIGINT,
                        @NewContactGroupId BIGINT;

                SELECT @IsContactGroup = IsSubmittedForGroup,
                       @CurContactGroupId = ContactGroupId,
                       @CurContactMasterId = ContactMasterId,
                       @oldSenderCellNo = SenderCellNo
                FROM SeenClientAnswerMaster
                WHERE Id = @AnswerMasterId;
                ...
                SET @GroupId =
                (
                    SELECT GroupId FROM AppUser WHERE Id = @NewAppUserId
                );
                ...
                BEGIN
                    ...
                    BEGIN
                        SET @ContactMasterIDs =
                        (
                            SELECT STRING_AGG(CAST(ContactMasterId AS NVARCHAR), ',')
                            FROM ContactGroupRelation
                            WHERE ContactGroupId = @CurContactGroupId
                                  AND IsDeleted = 0
                        ) + ',' + CAST(@NewAppUserContactMasterId AS NVARCHAR);
                        SET @ContactGroupName =
                        (
                            SELECT ContactGropName FROM ContactGroup WHERE Id = @CurContactGroupId
                        ) + ' and ' +
                        (
                            SELECT "Name" FROM AppUser WHERE Id = @NewAppUserId
                        );
                    END;
                    ELSE
                    BEGIN
                ...
                BEGIN
                    SET @ExistingGroupId =
                    (
                        SELECT dbo.fn_IsContactGroupExistsByContactMasterIds(@ContactMasterIDs)
                    );
                    ...
                    BEGIN
                        ...
                        VALUES
                        (@ContactGroupName, @GroupId, '', 1, GETUTCDATE(), @AppUserId, 0);
                        ...
                        BEGIN
                            ...
                            SELECT ContactMasterId,
                                   ...
                                   DeletedBy,
                                   IsDeleted
                            FROM ContactGroupRelation CGR
                            WHERE CGR.ContactGroupId = @CurContactGroupId
                            UNION
                            SELECT @NewAppUserContactMasterId,
                                   @NewContactGroupId,
                                   GETUTCDATE(),
                                   @AppUserId,
                                   NULL,
                    ...
                    VALUES
                    (@AMId, @NewAppUserContactMasterId, @oldSenderCellNo);

                    --SELECT  @SCACId = ISNULL(CAST(SCOPE_IDENTITY() AS BIGINT), 0);

                    DECLARE @SCACExId BIGINT = 0;
                    SET @SCACExId =
                    (
                        SELECT TOP 1
                            Id
                        FROM SeenClientAnswerChild
                        WHERE SeenClientAnswerMasterId = @AMId
                              AND IsDeleted = 0
                    );
                    ...
                    (
                        ...
                        FROM SeenClientAnswerChild
                        WHERE SeenClientAnswerMasterId = @AMId
                              AND ContactMasterId = @NewAppUserContactMasterId
                    );

                    INSERT INTO SeenClientAnswers
                    (
                        SeenClientAnswerMasterId,
    ...
    BEGIN
        DECLARE @TimeOfSet INT;
        IF @IsOut = 0
        BEGIN
            SELECT @TimeOfSet = TimeOffSet
            FROM dbo.AnswerMaster
            WHERE Id = @AMId;
        END;
        ELSE
        ...
        BEGIN
            SELECT @TransferFromEstablishment = E.EstablishmentName
            FROM dbo.Establishment E
                INNER JOIN dbo.SeenClientAnswerMaster SAM
        ...
        SET @TransferFromRefNo
            = CAST(LEFT(REPLICATE(0, 10 - LEN(@AnswerMasterId)) + CAST(@AnswerMasterId AS VARCHAR(50)), 10) AS VARCHAR(50));
        SET @TransferToRefNo
            = CAST(LEFT(REPLICATE(0, 10 - LEN(@AMId)) + CAST(@AMId AS VARCHAR(50)), 10) AS VARCHAR(50));
        ...
        BEGIN
            ...
            BEGIN
                SET @TransferFormSMSText = REPLACE(@TransferFormSMSText, '##[fromrefno]##', @TransferFromRefNo);
                SET @TransferFormSMSText = REPLACE(@TransferFormSMSText, '##[torefno]##', @TransferToRefNo);
                SET @TransferFormSMSText = REPLACE(@TransferFormSMSText, '##[fromusername]##', @TransferFromUsername);
                SET @TransferFormSMSText = REPLACE(@TransferFormSMSText, '##[tousername]##', @TransferToUsername);
                SET @TransferFormSMSText
                    = REPLACE(@TransferFormSMSText, '##[fromestablishment]##', @TransferFromEstablishment);
                SET @TransferFormSMSText
                    = REPLACE(@TransferFormSMSText, '##[toestablishment]##', @TransferToEstablishment);
        ...
        BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    SET @TransferFormEmailSubject = 'New Form transferred From ' + @TransferFromUsername;
                END;
                ELSE
                BEGIN
                    SET @TransferFormEmailSubject
                        = REPLACE(@TransferFormEmailSubject, '##[fromrefno]##', @TransferFromRefNo);
                    SET @TransferFormEmailSubject
                        = REPLACE(@TransferFormEmailSubject, '##[torefno]##', @TransferToRefNo);
                    SET @TransferFormEmailSubject
                        = REPLACE(@TransferFormEmailSubject, '##[fromusername]##', @TransferFromUsername);
                    SET @TransferFormEmailSubject
                        = REPLACE(@TransferFormEmailSubject, '##[tousername]##', @TransferToUsername);
                    SET @TransferFormEmailSubject
                        = REPLACE(@TransferFormEmailSubject, '##[fromestablishment]##', @TransferFromEstablishment);
                    SET @TransferFormEmailSubject
                        = REPLACE(@TransferFormEmailSubject, '##[toestablishment]##', @TransferToEstablishment);

                    SET @TransferFormEmailText
                        = REPLACE(@TransferFormEmailText, '##[fromrefno]##', @TransferFromRefNo);
                    SET @TransferFormEmailText = REPLACE(@TransferFormEmailText, '##[torefno]##', @TransferToRefNo);
                    SET @TransferFormEmailText
                        = REPLACE(@TransferFormEmailText, '##[fromusername]##', @TransferFromUsername);
                    SET @TransferFormEmailText
                        = REPLACE(@TransferFormEmailText, '##[tousername]##', @TransferToUsername);
                    SET @TransferFormEmailText
                        = REPLACE(@TransferFormEmailText, '##[fromestablishment]##', @TransferFromEstablishment);
                    SET @TransferFormEmailText
                        = REPLACE(@TransferFormEmailText, '##[toestablishment]##', @TransferToEstablishment);
                END;
