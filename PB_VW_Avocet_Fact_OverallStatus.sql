Create view PB_VW_Avocet_Fact_OverallStatus as

WITH cte AS (
SELECT * ,
  ROW_NUMBER() OVER (PARTITION BY ReferenceNo ORDER BY Statustime ASC) AS rn
FROM (
select distinct U.Id AS AppUserId,sh.ReferenceNo,sh.StatusDateTime as Statustime,convert(date,statusdatetime) as StatusDate,es.StatusName,U.Name as UserName,
 case when  StatusName='Planned for Today' then 1 when  StatusName='Arrived On-Site' then 2 when StatusName='Complete' then 3
 else 0 end as StatusSort from StatusHistory sh 
...
Where (G.Id=469 and EG.Id IN(4089,4349,4087)
...
union all
select distinct U.Id AS AppUserId,A.Id as ReferenceNo,DATEADD(minute,120,GETDATE()) as Statustime,convert(date,getdate()) as StatusDate,'Arrived On-Site' as StatusName,U.Name as UserName,
100 as StatusSort from StatusHistory sh 
inner join establishmentstatus es on sh.establishmentstatusid=es.id and es.IsDeleted=0
inner join Appuser U on U.id=Sh.UserId 
inner join (select Am.id from dbo.[Group] G
inner join EstablishmentGroup EG on G.id=EG.groupid
inner join Establishment E on  E.EstablishmentGroupId=EG.Id 
inner join SeenClientAnswerMaster AM on AM.EstablishmentId=E.id Where (G.Id=469 and EG.Id IN(4089,4349,4087)
ANd (AM.IsDeleted=0 or AM.IsDeleted=null)))A on A.id=Sh.referenceno
...
SELECT B.ReferenceNo, B.AppUserId,B.UserName,B.StatusName,B.Statustime,B.StatusSort,B. Endtime AS EndTime,A.[TotalTime] AS TotalTime from
