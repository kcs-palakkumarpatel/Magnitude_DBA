...
/*
EXEC dbo.AdvanceSearchFormDataOffline_OfflineAPI @EstablishmentId = '0',
@UserId = '0', -- varchar(max)
@ActivityId = 2101, -- bigint
@FromDate = '2019-11-11 00:00:00', -- datetime
@ToDate = '2020-11-10 23:59:59', -- datetime
@AppuserId = 1246,
@FormStatus = 

'Unresolved', -- varchar(10)
@IsResponse = 'true', -- varchar(5)
@ResponseType = 'All', -- varchar(15)
@LastServerDate = '2020-09-30 00:00:00'
...
Drop procedure [dbo].[AdvanceSearchFormDataOffline_OfflineAPI]
*/
--=============================================
CREATE PROCEDURE [dbo].[AdvanceSearchFormDataOffline_OfflineAPI]
...
BEGIN
	...
	SELECT @GroupType = ISNULL(Id,0) FROM dbo.AppUser
	WHERE GroupId IN (SELECT data FROM dbo.Split((SELECT KeyValue FROM dbo.AAAAConfigSettings WHERE KeyName = 'ExcludeGroupId'),',')) 
	AND id = @AppuserId
	...
	SET @SqlSelect1 = N'
        ...
        TimeOffSet ,CreatedOn ,Lastupdated, dbo.ChangeDateFormat(UpdatedOn,''dd/MMM/yyyy hh:mm AM/PM'') AS UpdatedOn, 
		[PI] as [PI],
		...
		UnreadAction, R.IsRecursion,Sum(CASE WHEN SmileType = ''Positive'' THEN 1 ELSE 0 END) OVER() AS PositiveCount ,Sum(CASE WHEN SmileType = ''Negative'' THEN 1 ELSE 0 END)  OVER() AS NegativeCount ,Sum(CASE WHEN SmileType = ''Neutral'' THEN 1 ELSE 0 END)  
OVER() AS PassiveCount ,
	...
	SET @SqlSelect11 =  CHAR(13) + N'FROM    ( SELECT    A.* ,
                                   ...
                                   ( SELECT CASE A.IsOut WHEN 1 THEN ISNULL(( SELECT CASE WHEN IsRecursion = 0 THEN 0 ELSE 1 END FROM dbo.SeenClientAnswerMaster WHERE   Id = A.ReportId ), 0) ELSE 0 END ) AS IsRecursion,
									...
									FROM  dbo.View_AllAnswerMaster_OfflineAPI AS A ';
    ...
    IF (IsNull(@LastServerDate,'') <> '')
    BEGIN
		SET @Filter += ' AND A.LastUpdated >= ''' + CAST(@LastServerDate AS Varchar(50)) + ''''
	END;
	...
	EXEC ('select A.SeenClientAnswerMasterId FROM  dbo.View_AllAnswerMaster_OfflineAPI AS A ' + @SqlSelect12 + @SqlSelect2 + @Filter + ' Group By A.SeenClientAnswerMasterId');

	
				
	DECLARE @var VARCHAR(MAX)
	SET @var  = (SELECT distinct STUFF((SELECT distinct ',' + CONVERT(VARCHAR(max),p1.[SeenClientAnswerMasterId]) FROM @Temp p1
	...
	SELECT * FROM (
		...
		isnull(Lastupdated ,0) AS Lastupdated ,
	...
	INSERT INTO dbo.ErrorLog
        ...
        VALUES
        (ERROR_LINE(),
         'dbo.AdvanceSearchFormDataOffline_OfflineAPI',
         N'Database',
         ERROR_MESSAGE(),
