...
-- =============================================
CREATE PROCEDURE [dbo].[ExportFeedbackData]
(
...
BEGIN
    SET @SearchQuery ='	AND (REPLACE(STR(Am.Id , 10), SPACE(1), ''0'') like ''%' + @search + '%'' 
										OR E.EstablishmentName LIKE ''%' + @search + '%'' 
										OR Am.EI LIKE ''%' + @search + '%'' 
										OR U.UserName LIKE ''%' + @search + '%'' 
										OR Am.SenderCellNo LIKE ''%' + @search + '%''
										OR dbo.ChangeDateFormat(Am.CreatedOn, ''dd/MMM/yyyy hh:mm AM/PM'') LIKE ''%' + @search + '%'' 
										OR ISNULL(ANS.Detail, '''') LIKE ''%'+ @search + '%'' 
										OR ISNULL(Q.ShortName, '''') LIKE ''%'+ @search + '%'' )'+ CHAR(10); 
END

IF (@FilterOn = 'Neutral') 
			SET @FilterOn = ' AM.IsPositive = ''Neutral'''; 
		ELSE IF (@FilterOn = 'Positive') 
			SET @FilterOn = ' AM.IsPositive = ''Positive'''; 
		ELSE IF (@FilterOn = 'Negative') 
			SET @FilterOn = ' AM.IsPositive = ''Negative'''; 
		ELSE IF @FilterOn = 'OutStanding' 
			SET @FilterOn = ' ISNULL(AM.IsOutStanding,0) = 1 '; 
		ELSE IF @FilterOn = 'Resolved' 
			SET @FilterOn = ' AM.IsResolved = ''Resolved'''; 
		ELSE IF @FilterOn = 'UnResolved' 
			SET @FilterOn = ' AM.IsResolved = ''UnResolved'''; 
		ELSE IF @FilterOn = 'Actioned' 
			SET @FilterOn = ' ISNULL(AM.IsActioned,0) = 1'; 
		ELSE IF @FilterOn = 'UnActioned' 
			SET @FilterOn = ' ISNULL(AM.IsActioned,0) = 0'; 
		ELSE IF @FilterOn = 'Transferred' 
			SET @FilterOn = ' ISNULL(AM.IsTransferred,0) = 1'; 
		ELSE IF @FilterOn = 'In' and @IsOut = 1 
			SET @FilterOn = ' 2 = 1'; 
        ELSE IF @FilterOn = 'In' and @IsOut = 0 
			SET @FilterOn = '';	  
		ELSE IF @FilterOn = 'Out' and @IsOut = 0 
			SET @FilterOn = ' 2 = 1'; 
		ELSE IF @FilterOn = 'Out' and @IsOut = 1 
			SET @FilterOn = ''; 
     ...
     IF @IsOut = 0
			...
			SET @SqlSelect2 = CHAR(10) + CHAR(13) +' FROM    dbo.AnswerMaster AS Am
			...
			SET	@SqlSelect3 = CHAR(10) + CHAR(13) +' WHERE   Am.IsDeleted = 0
				        AND Am.EstablishmentId IN ( SELECT Data FROM dbo.split(''' + CONVERT(NVARCHAR(MAX), @EstablishmentId) + ''', '',''))
						AND Am.AppuserId in ( SELECT Data FROM dbo.split(''' + CONVERT(NVARCHAR(MAX), @UserId) + ''', '',''))
						AND Am.CreatedOn BETWEEN ''' + CONVERT(NVARCHAR(50), @FromDate) + ''' AND ''' + CONVERT(NVARCHAR(50), DATEADD(dd,1,@ToDate)) + '''
						AND Q.IsDeleted = 0
        ...
        ELSE
            BEGIN
               ---## Capture Form Data ##';
				...
				DECLARE @SqlSelect7 VARCHAR(MAX)= '' ,@SqlSelect8 VARCHAR(MAX) = '',@SqlSelect9 VARCHAR(MAX)= '' ,@SqlSelect4 VARCHAR(MAX)= '' ,@SqlSelect5 VARCHAR(MAX) = '',@SqlSelect6 VARCHAR(MAX)= '' ;
				...
				SET @SqlSelect1 = 'SELECT  Q.Position, Am.Id AS AnswerMasterID, Q.Id AS QuestionID,E.EstablishmentName AS [Establishment Name] ,
				        ...
				        CASE Am.IsTransferred WHEN 1 THEN ISNULL(U.Name, '''') ELSE '''' END AS [Transfer To User],
				        ISNULL(Am.IsDisabled, 0) AS [Disabled]' 
				SET @SqlSelect2 = CHAR(10) +' FROM  dbo.SeenClientAnswerMaster  AS Am
				        INNER JOIN dbo.Establishment AS E ON Am.EstablishmentId = E.Id
				        ...
				        LEFT OUTER JOIN dbo.AppUser AS TransferFromUser ON TransferFromAM.AppUserId = TransferFromUser.Id
				        LEFT OUTER JOIN dbo.AppUser AS TransferByUser ON Am.CreatedBy = TransferByUser.Id'
				SET	@SqlSelect3 = CHAR(10) + ' WHERE   Am.IsDeleted = 0
				        AND Am.EstablishmentId IN ( SELECT Data FROM dbo.split(''' + CONVERT(NVARCHAR(MAX), @EstablishmentId) + ''', '','')) 
						AND Am.AppuserId in ( SELECT Data FROM dbo.split(''' + CONVERT(NVARCHAR(MAX), @UserId) + ''', '',''))
						AND Am.CreatedOn BETWEEN ''' + CONVERT(NVARCHAR(50), @FromDate) + ''' AND ''' + CONVERT(NVARCHAR(50), DATEADD(dd,1,@ToDate)) + '''
						AND Q.IsDeleted = 0
						...
						AND Q.IsActive = 1
						AND Q.ContactQuestionId IS NULL ' + CHAR(10)
			SET @SqlSelect4 = 'UNION ALL '+CHAR(10) +' SELECT  Q.Position, Am.Id AS AnswerMasterID, Q.Id AS QuestionID,E.EstablishmentName AS [Establishment Name] ,
				        ISNULL(U.Name, '''') AS [User Name] ,
				        dbo.ChangeDateFormat(DATEADD(MINUTE,E.TimeOffSet,Am.CreatedOn),''dd-MMM-yyyy hh:mm AM/PM'') AS [Capture Date],
				        ISNULL(Am.SeenClientAnswerMasterId, '''') AS [Out Reference No] ,
				        Am.Id AS [Reference No] ,
				        ...
				        CASE Am.IsTransferred WHEN 1 THEN ISNULL(U.Name, '''') ELSE '''' END AS [Transfer To User],
				        ISNULL(Am.IsDisabled, 0) AS [Disabled]' 
				SET @SqlSelect5 = CHAR(10) +' FROM  dbo.SeenClientAnswerMaster  AS Am
				        INNER JOIN dbo.Establishment AS E ON Am.EstablishmentId = E.Id
				        INNER JOIN dbo.SeenClient AS Qr ON Qr.Id = Am.SeenClientId
				        INNER JOIN dbo.SeenClientQuestions AS Q ON Q.SeenClientId = Am.SeenClientId
				        LEFT JOIN dbo.ContactQuestions AS CQ ON CQ.Id = Q.ContactQuestionId AND CQ.IsDeleted = 0
				        ...
				        LEFT OUTER JOIN dbo.AppUser AS TransferFromUser ON TransferFromAM.AppUserId = TransferFromUser.Id
				        LEFT OUTER JOIN dbo.AppUser AS TransferByUser ON Am.CreatedBy = TransferByUser.Id'
				SET	@SqlSelect6 = CHAR(10) + ' WHERE   Am.IsDeleted = 0
				        AND Am.EstablishmentId IN ( SELECT Data FROM dbo.split(''' + CONVERT(NVARCHAR(MAX), @EstablishmentId) + ''', '','')) 
						AND Am.AppuserId in ( SELECT Data FROM dbo.split(''' + CONVERT(NVARCHAR(MAX), @UserId) + ''', '',''))
						AND Am.CreatedOn BETWEEN ''' + CONVERT(NVARCHAR(50), @FromDate) + ''' AND ''' + CONVERT(NVARCHAR(50), DATEADD(dd,1,@ToDate)) + '''
						AND Q.IsDeleted = 0
				...
				SET @SqlSelect7 ='UNION ALL '+CHAR(10) +' SELECT  Q.Position, Am.Id AS AnswerMasterID, Q.Id AS QuestionID,E.EstablishmentName AS [Establishment Name] ,
				        ISNULL(U.Name, '''') AS [User Name] ,
				       dbo.ChangeDateFormat(DATEADD(MINUTE,E.TimeOffSet,Am.CreatedOn),''dd-MMM-yyyy hh:mm AM/PM'') AS [Capture Date],
				        ISNULL(Am.SeenClientAnswerMasterId, '''') AS [Out Reference No] ,
				        Am.Id AS [Reference No] ,
				        ...
				        CASE Am.IsTransferred WHEN 1 THEN ISNULL(U.Name, '''') ELSE '''' END AS [Transfer To User],
				        ISNULL(Am.IsDisabled, 0) AS [Disabled]' 
				SET @SqlSelect8 = CHAR(10) +' FROM  dbo.SeenClientAnswerMaster  AS Am
				        INNER JOIN dbo.Establishment AS E ON Am.EstablishmentId = E.Id
				        INNER JOIN dbo.SeenClient AS Qr ON Qr.Id = Am.SeenClientId
				        INNER JOIN dbo.SeenClientQuestions AS Q ON Q.SeenClientId = Am.SeenClientId
				        LEFT JOIN dbo.ContactQuestions AS CQ ON CQ.Id = Q.ContactQuestionId AND CQ.IsDeleted = 0
				        ...
				        LEFT OUTER JOIN dbo.AppUser AS TransferFromUser ON TransferFromAM.AppUserId = TransferFromUser.Id
				        LEFT OUTER JOIN dbo.AppUser AS TransferByUser ON Am.CreatedBy = TransferByUser.Id'
				SET	@SqlSelect9 = CHAR(10) + ' WHERE   Am.IsDeleted = 0
				        AND Am.EstablishmentId IN ( SELECT Data FROM dbo.split(''' + CONVERT(NVARCHAR(MAX), @EstablishmentId) + ''', '','')) 
						AND Am.AppuserId in ( SELECT Data FROM dbo.split(''' + CONVERT(NVARCHAR(MAX), @UserId) + ''', '',''))
						AND Am.CreatedOn BETWEEN ''' + CONVERT(NVARCHAR(50), @FromDate) + ''' AND ''' + CONVERT(NVARCHAR(50), DATEADD(dd,1,@ToDate)) + '''
						AND Q.IsDeleted = 0
