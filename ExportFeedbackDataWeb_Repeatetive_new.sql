...
-- =============================================  
CREATE PROC dbo.ExportFeedbackDataWeb_Repeatetive_new  
(  
...
BEGIN  
  ...
  INNER JOIN (SELECT * FROM dbo.Split(''' + @UserId  
    ...
    ELSE  
    BEGIN  
        INSERT INTO @ResultReportIdJoin  
        SELECT DISTINCT  
            SeenClientAnswerMasterId,  
            COUNT(1)  
        FROM @ResultReportId A  
            INNER JOIN SeenClientAnswers B  
                ON A.ReportId = B.SeenClientAnswerMasterId  
        GROUP BY SeenClientAnswerMasterId,  
                 SeenClientAnswerChildId,  
                 RepeatCount  
        HAVING RepeatCount <> 0  
        ORDER BY 2 DESC;  
        ...
        (  
            ...
            [RepeatCount] INT,  
            RepetitiveGroupName NVARCHAR(MAX),  
        ...
        FROM  
        (  
            --SELECT *,ROW_NUMBER() OVER (PARTITION BY xy.SeenClientAnswerMasterId,xy.QuestionId ORDER BY xy.SeenClientAnswerMasterId,xy.QuestionId )  AS RepeatCount  
            --FROM (  
            ...
            (  
                ...
                FROM dbo.SeenClientAnswers A  
                    JOIN dbo.SeenClientQuestions B  
        ...
        SELECT SeenClientAnswerMasterId,  
               ...
               B.ShortName  
        FROM dbo.SeenClientAnswers A  
            INNER JOIN dbo.SeenClientQuestions B  
                ON A.QuestionId = B.Id  
                   AND B.SeenClientId = @SeenClientID  
                   AND B.IsDeleted <> 1  
        WHERE SeenClientAnswerMasterId IN (  
...
--		CREATE TABLE #pivot ( [Position] int, [AnswerMasterID] bigint, [QuestionID] bigint, [Establishment Name] nvarchar(max), [User Name] nvarchar(max), [Capture Date] nvarchar(max), [Out Reference No] bigint, [Reference No] bigint, [PI] decimal(18,2), [Contact Group Name] nvarchar(50), [Question] nvarchar(max), [Answer] nvarchar(max), [Status] nvarchar(max), [Resolution Comments] nvarchar(max), [Is Actioned] bit, [Is Transferred] bit, [Transfer From User] nvarchar(max), [Transfer To User] nvarchar(max), [Disabled] bit, [RepeatCount] int )
--INSERT INTO #Pivot
		SELECT [Position],  
               [AnswerMasterID],  
               [QuestionID],  
        ...
        (  
            SELECT Q.Position,  
                   ...
                   ISNULL(  
                   (  
                       SELECT ContactGropName  
                       FROM dbo.ContactGroup  
                       WHERE Id IN (  
                                       SELECT ContactGroupId FROM dbo.SeenClientAnswerMaster WHERE Id = Am.Id  
                                   )  
                   ),  
                   ''  
                         ) AS [Contact Group Name],  
                   ANS.Question,  
                   CASE Q.QuestionTypeId  
                       WHEN 8 THEN  
                   (CASE  
            ...
            FROM dbo.SeenClientAnswerMaster AS Am  
                ...
                LEFT JOIN dbo.ContactQuestions AS CQ  
 ...
 dbo.ChangeDateFormat(ANS.Detail, 'dd/MMM/yyyy')  
        ...
        WHERE TM.Question IS NOT NULL ----AND TM.[Reference No] != 194103  
        ORDER BY TM.AnswerMasterID,  
                 TM.RepetitiveGroupId,  
                 TM.RepeatCount,  
                 TM.Position ASC;  
...
--                (
--                    SELECT 
--                        ',' + CONVERT(VARCHAR(MAX),'['+ p1.Question+']')
--                    FROM #Pivot p1
--					GROUP BY p1.Question,RepeatCount
--					 ORDER BY p1.RepeatCount
--                    FOR XML PATH('')
--                ),
--                1,
--                1,
