...
-- Description:	<Description,,DeleteSupplier>
-- Call SP    :	DeleteSupplier
-- =============================================
    ...
    BEGIN
        ...
        WHILE @Start <= @Total 
            BEGIN
                ...
                FROM    @Tbl
                WHERE   Id = @Start
                EXEC dbo.IsReferenceExists N'Supplier', @Id, @IsUsed OUTPUT
                IF @IsUsed = 0 
                    BEGIN
                        UPDATE  dbo.[Supplier]
                        SET     IsDeleted = 1 ,
                        ...
                        VALUES  ( @DeletedBy ,
                                  @PageId ,
                                  'Delete record in table Supplier' ,
                                  'Supplier' ,
                                  @Id ,
                                  GETUTCDATE() ,
                                  @DeletedBy ,
                ...
                ELSE 
                    BEGIN
                        SET @Count += 1
                        SELECT  @Result = @Result + ', ' + SupplierName
                        FROM    dbo.[Supplier]
                        WHERE   [Id] = @Id
                    END
                SET @Start += 1
