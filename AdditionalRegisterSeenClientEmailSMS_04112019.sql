...
-- Call SP:		AdditionalRegisterSeenClientEmailSMS_04112019 418715,0,1373,24173,4615,'AA','',false
-- =============================================
CREATE PROCEDURE [dbo].[AdditionalRegisterSeenClientEmailSMS_04112019]
...
BEGIN
    ...
    PRINT @SmileType;
    PRINT @CaptureDate;
    PRINT @ISAdditionalCaptureEmail;
    PRINT @AdditionalCaptureEmails;
    PRINT @AdditionalCaptureEmailSubject;
    PRINT @AdditionalCaptureEmailBody;
    PRINT @ISAdditionalCaptureSMS;
    PRINT @AdditionalCaptureMobile;
    PRINT @AdditionalCaptureSMSBody;
    ...
    SELECT SA.QuestionId,
           CASE QuestionTypeId
               WHEN 8 THEN
                   FORMAT((DATEADD(MINUTE, 0, Detail)), 'dd/MMM/yyyy', 'en-US')
               WHEN 9 THEN
                   FORMAT((DATEADD(MINUTE, 0, Detail)), 'hh:mm AM/PM', 'en-US')
               WHEN 22 THEN
                   FORMAT((DATEADD(MINUTE, 0, Detail)), 'dd/MMM/yyyy hh:mm AM/PM', 'en-US')
               WHEN 17 THEN
                   dbo.GetFileTypeQuestionImageString(Detail, 1)
               WHEN 1 THEN
               (
    ...
    BEGIN
		...
		SELECT @ISAdditionalCaptureEmail 
           SELECT  @AdditionalCaptureEmailBody 
