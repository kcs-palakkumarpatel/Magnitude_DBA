...
-- =============================================
CREATE PROCEDURE dbo.InsertOrUpdateAppUserEstablishment_07202020_Duplicate  @Establishment XML
AS
BEGIN
    ...
    ----IF (@EstablishmentType = 'Customer')
    ----BEGIN
    ----INSERT INTO #DeleteEstabilshment
    ----        ( EstablishmentId )
    ----SELECT AUE.EstablishmentId FROM dbo.AppUserEstablishment AUE WHERE AUE.EstablishmentId NOT IN (SELECT EstablishmentId FROM #ExistTable) AND AUE.AppUserId = @AppUserId

    ----UPDATE AUE SET IsDeleted =  1,
    ----                  DeletedBy = @UserId ,
    ----                  DeletedOn = GETUTCDATE() FROM dbo.AppUserEstablishment AUE INNER JOIN #DeleteEstabilshment E ON E.EstablishmentId = AUE.EstablishmentId AND AUE.AppUserId = @AppUserId
    ----			   AND AUE.EstablishmentType = 'Customer'
    ----END
    ----ELSE
    ----   BEGIN
    ----INSERT INTO #DeleteEstabilshment
    ...
    ----                  DeletedBy = @UserId ,
    ----                  DeletedOn = GETUTCDATE() FROM dbo.AppUserEstablishment AUE INNER JOIN #DeleteEstabilshment E ON E.EstablishmentId = AUE.EstablishmentId AND AUE.AppUserId = @AppUserId
    ----			   AND AUE.EstablishmentType = 'Sales'
    ----END
