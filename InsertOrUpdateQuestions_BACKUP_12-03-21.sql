...
-- Description:	<Description,,InsertOrUpdateQuestions>
-- Call SP    :	InsertOrUpdateQuestions
-- =============================================
CREATE PROCEDURE [dbo].[InsertOrUpdateQuestions_BACKUP_12-03-21]
(
    @Id BIGINT,
...
BEGIN
    ...
    BEGIN
        ...
        VALUES
        (@QuestionnaireId, @Position, @QuestionTypeId, @QuestionTitle, @ShortName, @IsActive, @Required,
         @IsDisplayInSummary, @IsDisplayInDetail, @MaxLength, @Hint, @EscalationRegex, @OptionsDisplayType,
         @SeenClientQuestionIdRef, @IsTitleBold, @IsTitleItalic, @IsTitleUnderline, @TitleTextColor, @TableGroupName,
         @Margin, @FontSize, @ImagePath, @Weight, @WeightForYes, @WeightForNo, GETUTCDATE(), @UserId, 0,
         @EscalationValue, @DisplayInGraphs, @DisplayInTableView, @IsCommentCompulsory, @MultipleRoutingValue,
         @IsAnonymous, @ContactQuestionIdRef, @AllowDecimal, @IsRepetitive, @QuestionsGroupNo, @QuestionsGroupName,
         @IsSignature, @ImageHeight, @ImageWidth, @ImageAlign, @CalculationOptionId, @SummarayOptionId,
         @DefaultQuestion, @IsRoutingOnGroup, @IsRequireHTTPHeader, @IsValidateUsingQR);
