...
-- Create On : 04-Jun-2021
-- Description : Get App User By Establishment Id
-- GetAppUserListByEstablishmentId 8743,4567,'0'
-- =============================================
CREATE PROCEDURE dbo.GetAppUserListByEstablishmentId_111821
    @ActivityId BIGINT,
    @AppUserId BIGINT,
...
SET NOCOUNT ON;
    ...
    SELECT AMUR.ManagerUserId AS AppUserId,
           ...
           (
               ...
               WHERE CD.QuestionTypeId = 10
                     AND AU.Email = CD.Detail 
					 AND CD.Detail <>'' AND CD.Detail IS NOT NULL                    
               ORDER BY CD.ContactMasterId DESC
           ) AS ContactMasterId
    ...
    WHERE (
              @SearchText = ''
              OR AU.Name LIKE '%' + @SearchText + '%'
          ) AND AU.Email <>'' AND AU.Email IS NOT NULL
