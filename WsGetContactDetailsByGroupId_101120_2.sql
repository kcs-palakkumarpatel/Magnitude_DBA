...
drop procedure WsGetContactDetailsByGroupId_101120
exec WsGetContactDetailsByGroupId_101120 534,5830,'2020-11-01 00:00:00'
exec WsGetContactDetailsByGroupId 534,5830
*/
CREATE PROCEDURE [dbo].[WsGetContactDetailsByGroupId_101120_2]
	@GroupId BIGINT ,
    @AppUserId BIGINT,
...
BEGIN
	...
	BEGIN
        ...
        SELECT  DISTINCT
            ...
            R.IsGroup,
			R.LastUsedOn
		FROM (SELECT TOP 1000000 ISNULL(SAM.ContactMasterId, SAM.ContactGroupId) AS ContactMasterOrGroupId ,
				SAM.SeenClientId ,
	...
	BEGIN
		...
		FROM ( SELECT TOP 1000000
				...
				LEFT OUTER JOIN dbo.ContactGroup AS CG ON CG.Id = SAM.ContactGroupId
				WHERE 
				--SAM.SeenClientId = EG.SeenClientId
				 ( ISNULL(CM.GroupId, 0) = @GroupId OR ISNULL(CG.GroupId, 0) = @GroupId )
				AND ISNULL(CM.IsDeleted, 0) = 0
                AND ISNULL(CG.IsDeleted, 0) = 0
				AND ( ISNULL(CM.UpdatedON,CM.CreatedOn) >= @LastServerDate OR ISNULL(CG.UpdatedON,CG.CreatedON) >= @LastServerDate)
                GROUP BY  SAM.SeenClientId ,
	...
	Where LastUsedOn >= @LastServerDate
	UNION
	SELECT ContactId,
