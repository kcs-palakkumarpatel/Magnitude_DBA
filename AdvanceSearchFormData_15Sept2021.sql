...
EXEC dbo.AdvanceSearchFormData @EstablishmentId = '0', -- varchar(max)
    ...
    @FromDate = '2017-08-02 06:18:39', -- datetime
    @ToDate = '2018-12-13 23:18:39', -- datetime
    @FilterOn = N'', -- nvarchar(50)
    @QuestionSearch = N'', -- nvarchar(max)
    @AppuserId = 1243, -- int
    @ReportId = 0, -- int
    @FormStatus = 'All', -- varchar(10)
    ...
    @IsResponse = 'false', -- varchar(5)
    @ResponseType = 'All', -- varchar(15)
	@IsFlag = 'false'
	*/

--=============================================
CREATE PROCEDURE [dbo].[AdvanceSearchFormData_15Sept2021]
...
BEGIN
    SET NOCOUNT ON;
    --SET @Rows = 10;
    --IF  @ActivityId IN (1115,1153,1155,1157,1159,1167,1269,1927)
    --BEGIN
    --    SET @Rows = 10;
    --END
    ...
    DECLARE @PIIn VARCHAR(5);
    DECLARE @PIOut VARCHAR(5);

    SELECT @PIIn = PIStatus,
           @PIOut = PIOutStatus
    ...
    SELECT @ActionFilter = ' AND A.ReportId IN (select Id from #ActionFilterId)';
    ...
    DECLARE @Establishment TABLE (EstablishemtnId BIGINT);
        ...
        IF (
           (
               ...
               WHERE QuestionTypeId IN ( 1, 5, 6, 7, 18, 21 )
                     AND SeenClientId =
                     (
                         SELECT SeenClientId FROM dbo.EstablishmentGroup WHERE Id = @ActivityId
                     )
                     AND [Required] = 1
                     AND IsDeleted = 0
           ) > 0
        ...
        IF (
           (
               ...
               WHERE QuestionTypeId IN ( 1, 5, 6, 7, 18, 21 )
                     AND QuestionnaireId =
                     (
                         SELECT QuestionnaireId FROM dbo.EstablishmentGroup WHERE Id = @ActivityId
                     )
                     AND [Required] = 1
                     AND IsDeleted = 0
           ) > 0
    ...
    SET @SqlselectCount
        = N'SELECT DISTINCT Sum(CASE WHEN SmileType = ''Positive'' THEN 1 ELSE 0 END) OVER() AS PositiveCount ,Sum(CASE WHEN SmileType = ''Negative'' THEN 1 ELSE 0 END)  OVER() AS NegativeCount ,Sum(CASE WHEN SmileType = ''Neutral'' THEN 1 ELSE 0 END)  OVER() AS PassiveCount ,
    ...
    BEGIN
		...
		CASE WHEN IsOut = 1 THEN IIF(''' + @PIOut + ''' = 1,[PI],-100) ELSE IIF(''' + @PIIn
              + ''' = 1,[PI],-100) END AS [PI],
		...
		TransferByUserId ,TransferFromUserId ,dbo.AnswerDetails(CASE IsOut WHEN 0 THEN ''Answers'' ELSE ''SeenClientAnswers'' END, ReportId) AS DisplayText , dbo.ConcateString(''ContactSummary'', ContactMasterId) AS ContactDetails ,
						...
						UnreadAction, R.IsRecursion,0 AS PositiveCount ,0 NegativeCount ,0 AS PassiveCount ,0 AS UnresolvedCount,
				0 AS ResolvedCount,0 AS ActionedCount ,0 AS UnActionedCount ,0 AS TransferredCount ,0 AS OutstandingCount ,UnreadAction AS AlertUnreadCountCount ,
				...
				(SELECT dbo.DifferenceDatefun(ISNULL(A.StatusTime,GETUTCDATE()),DATEADD(MINUTE, A.TimeOffSet,GETUTCDATE()))) AS	R.StatusCounter   ';
        SET @SqlSelect11
            = CHAR(13)
              + N'FROM    ( SELECT    A.* ,
									0 AS UnreadAction,
              ...
              + IIF(@OrderBy = ' Order By R.ActionDate Desc',
                    ...
                    + CONVERT(VARCHAR(10), @AppuserId) + ' ORDER BY CreatedOn DESC)))',
                    IIF(@OrderBy = ' Order By R.ActionDateEveryone Desc',
                        '(DATEADD(MINUTE, A.TimeOffSet, ( SELECT TOP 1 CreatedOn FROM   dbo.PendingNotificationWeb WHERE  IsDeleted = 0 AND  ISNULL(RefId, 0) = A.ReportId AND ModuleId IN (11, 12) ORDER BY CreatedOn DESC)))',
                        'A.CreatedOn')) + ' DESC ) AS RowNum,
									CASE COUNT(1) OVER ( PARTITION BY 1 ) / ' + CONVERT(NVARCHAR(10), @Rows)
              + ' WHEN 0 THEN 1 ELSE ( COUNT(1) OVER ( PARTITION BY 1 ) / ' + CONVERT(NVARCHAR(10), @Rows)
              + ' ) + 1 END AS TotalPage,
									DATEADD(MINUTE, A.TimeOffSet, ( SELECT TOP 1 CreatedOn FROM   dbo.PendingNotificationWeb WHERE IsDeleted = 0 AND ISNULL(RefId, 0) = A.ReportId AND ModuleId IN (11, 12) AND ISNULL(AppUserId, 0) = '
              + CONVERT(VARCHAR(10), @AppuserId)
              ...
              + ' AS ActionTo ,
									...
									SeenClientAnswerMasterId = A.ReportId) AND SeenClientAnswerMasterId = A.ReportId) = 1,1,0) ELSE IIF((SELECT TOP 1 IsFeedBackSubmitted FROM dbo.FeedbackOnceHistory WHERE SeenClientAnswerMasterId = A.ReportId ORDER BY id DESC) = 1,1,0) end as FeedbackSubmitted,F.IsFlag '
              + CHAR(13) + '
									FROM      dbo.View_AllAnswerMaster AS A ';
        SET @SqlSelect12 += CHAR(13) + 'INNER JOIN (SELECT * FROM dbo.Split(''' + @EstablishmentId
                            + ''', '','')) AS E ON A.EstablishmentId = E.Data OR ''' + @EstablishmentId
                            + ''' = ''0''
                                    INNER JOIN (SELECT * FROM dbo.Split(''' + @UserId
    ...
    SET @SqlSelect2 += CHAR(13)
                       ...
                       + CHAR(13)
                       + ' 	LEFT OUTER JOIN dbo.FlagMaster AS F ON F.ReportId = A.ReportId AND F.Type IN (1,2) AND F.AppUserId = '''
                       + CONVERT(VARCHAR(10), @AppuserId) + '''';

    SET @Filter += CHAR(13) + ' WHERE CAST(A.CreatedOn AS DATE) BETWEEN CAST('''
                   + dbo.ChangeDateFormat(@FromDate, 'dd MMM yyyy')
                   + ''' AS DATE) 
			And Cast(dbo.ChangeDateFormat((DateAdd(Minute,A.TimeOffset,''' + CONVERT(VARCHAR(25), @ToDate)
                   + ''')),''dd MMM yyyy'') as Date)
    ...
    IF (@ResponseType != 'All')
    BEGIN
        DECLARE @Temp TABLE ([SeenClientAnswerMasterId] [BIGINT] NOT NULL);
    ...
    SET PositiveCount =
        (
            SELECT PositiveCount FROM @ResultCount
        );
    UPDATE @Result
    SET NegativeCount =
        (
            SELECT NegativeCount FROM @ResultCount
        );
    UPDATE @Result
    SET PassiveCount =
        (
            SELECT PassiveCount FROM @ResultCount
        );
    UPDATE @Result
    SET UnresolvedCount =
        (
            SELECT UnresolvedCount FROM @ResultCount
        );
    UPDATE @Result
    SET ResolvedCount =
        (
            SELECT ResolvedCount FROM @ResultCount
        );
    UPDATE @Result
    SET ActionedCount =
        (
            SELECT ActionedCount FROM @ResultCount
        );
    UPDATE @Result
    SET UnActionedCount =
        (
            SELECT UnActionedCount FROM @ResultCount
        );
    UPDATE @Result
    SET TransferredCount =
        (
            SELECT TransferredCount FROM @ResultCount
        );
    UPDATE @Result
    SET OutstandingCount =
        (
            SELECT OutstandingCount FROM @ResultCount
        );
    UPDATE @Result
    SET AlertUnreadCountCount =
        (
            SELECT AlertUnreadCountCount FROM @ResultCount
        );
    ...
    BEGIN
        ...
        BEGIN
            SELECT CONVERT(NVARCHAR(15), '') AS ReportId,
                   ...
                   CONVERT(BIT, 0) AS IsFlag,
                   CONVERT(NVARCHAR(MAX), '') AS LastChat,
                   CONVERT(NVARCHAR(MAX), '') AS LastChatBy,
                   CONVERT(BIGINT, 0) AS StatusId,
                   CONVERT(NVARCHAR(MAX), '') AS StatusName,
                   CONVERT(NVARCHAR(MAX), '') AS StatusImage,
                   CONVERT(DATETIME, GETDATE()) StatusTime,
                   CONVERT(NVARCHAR(MAX), '') StatusCounter;
        END;
    END;
    ELSE
    BEGIN
        IF
        (
            SELECT COUNT(1) AS TotRec FROM @Result
        ) > 0
        BEGIN
            SELECT *
            FROM
