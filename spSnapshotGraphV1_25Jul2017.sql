...
-- =============================================
CREATE PROCEDURE dbo.spSnapshotGraphV1_25Jul2017
                ...
                ELSE
					...
					BEGIN
					 INSERT  INTO @Result
                                ...
                                SELECT id,AVG(T.Detail),SUM(T.Counts),AVG(T.PI),0,0,0,COUNT(T.ReportId) 
								FROM ( SELECT  Q.Id ,
                                        ...
                                        0 AS BenchmarkPI,
										SUM(CASE ISNULL(A.Detail,'') WHEN '' THEN 0 ELSE 1 END) AS ReportId
                                FROM    dbo.View_SeenClientAnswerMaster AS Am
                                        INNER JOIN dbo.SeenClientAnswers AS A ON Am.ReportId = A.SeenClientAnswerMasterId
                                        INNER JOIN dbo.SeenClientQuestions Q ON A.QuestionId = Q.Id
                                                              AND Q.DisplayInGraphs = 1
					...
					BEGIN
                        ...
                        INSERT  INTO @Result
                                ...
                                FROM    dbo.View_SeenClientAnswerMaster AS Am
                                        INNER JOIN dbo.SeenClientAnswers AS A ON Am.ReportId = A.SeenClientAnswerMasterId
                                        INNER JOIN dbo.SeenClientQuestions Q ON A.QuestionId = Q.Id AND Q.DisplayInGraphs = 1
                                        INNER JOIN ( SELECT Data FROM   dbo.Split(@EstablishmentId, ',') ) AS RE ON ( RE.Data = Am.EstablishmentId OR @EstablishmentId = '0' )
                                        INNER JOIN ( SELECT Data FROM   dbo.Split(@QuestionIdList, ',') ) AS RQ ON RQ.Data = Q.Id
                            ...
                            BEGIN
                                ...
                                INSERT  INTO @FinalResult
                                        ...
                                        SELECT  @FromDate AS StartDate ,
                                                ...
                                                ROUND(( ( ISNULL(@ToatlWaitage, 0) - ISNULL(@YBScore, 0) ) / CASE ISNULL(@ToatlWaitage, 0) WHEN 0 THEN 1 ELSE ISNULL(@ToatlWaitage, 0) END * 100 ), 0) AS PerformanceBenchmarkScore ,
                                                @MinRank AS MinRank ,
                                                @MaxRank AS MaxRank ,
                                                @DisplayType AS GroupType,
												ISNULL(@TotalEntry,0)
        ...
        ELSE
          ...
          Am.ReportId;
            ...
            ',')
                ...
                SELECT  @TotalEntry = COUNT(1) FROM    @FinalValue;
