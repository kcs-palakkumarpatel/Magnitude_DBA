...
-- Description:	<Description,,DeleteClient>
-- Call SP    :	DeleteClient
-- =============================================
    ...
    BEGIN
        ...
        WHILE @Start <= @Total 
            BEGIN
                ...
                FROM    @Tbl
                WHERE   Id = @Start
                EXEC dbo.IsReferenceExists N'Client', @Id, @IsUsed OUTPUT
                IF @IsUsed = 0 
                    BEGIN
                        UPDATE  dbo.[Client]
                        SET     IsDeleted = 1 ,
                ...
                ELSE 
                    BEGIN
                        SET @Count += 1
                        SELECT  @Result = @Result + ', ' + ClientName
                        FROM    dbo.[Client]
                        WHERE   [Id] = @Id
                    END
                SET @Start += 1
