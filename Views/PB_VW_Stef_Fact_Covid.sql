CREATE view PB_VW_Stef_Fact_Covid as
...
and CD.contactQuestionId=3356
)+' '+(SELECT TOP 1 cd.Detail FROM ContactDetails CD WHERE   CD.ContactMasterId=(case when Am.IsSubmittedForGroup=1 then SAC.ContactMasterId else AM.ContactMasterId end) and CD.IsDeleted = 0 and CD.Detail<>'' 
and CD.contactQuestionId=3359
...
from dbo.[Group] G
inner join EstablishmentGroup EG on G.id=EG.groupid and G.Id=552 and EG.Id=6075
...
[Sore Throat],
[Headache],
[Household] as [Household Members],
[Tiredness],
[Shortness of Breat],
[Aches and pains],
[Diarrhoea],
