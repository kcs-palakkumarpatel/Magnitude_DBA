...
-- Description:	
-- Call SP:			dbo.GetTableViewGraph 1243,1941,'0', '0','01 sep 2017','31 Aug 2018',1,0,'All','All','','','',0,0,0,0,0,'',0,'14147','Environment'
-- =============================================
CREATE PROCEDURE [dbo].[GetTableViewGraph]
    (
	 @AppUserId BIGINT ,
      @ActivityId BIGINT ,
...
AS
	  ...
	  DECLARE @SellectAllEstablishment INT
                    ...
                    BEGIN
                        INSERT  INTO @Result
                                ...
                                FROM    dbo.View_AnswerMaster AS AM
                                        INNER JOIN dbo.Answers AS A ON AM.ReportId = A.AnswerMasterId
                                        INNER JOIN dbo.Questions AS Q ON Q.Id = A.QuestionId
										INNER JOIN (SELECT Data FROM dbo.Split(@OptionList,',')) AS O ON O.Data IN (SELECT data FROM dbo.Split(A.OptionId,','))
                ...
                ELSE
                    BEGIN
                        ...
                        INSERT  INTO @Result
                                ...
                                SELECT  Q.Id ,
                                        sum(CAST(A.Detail AS DECIMAL(18,2))) / CASE MAX(A.RepeatCount) WHEN 0 THEN 1 ELSE MAX(A.RepeatCount) end AS details ,
...
PRINT '==================='
               ...
               IF (@QuestionTypeId = 19 OR @QuestionTypeId = 7)
			        BEGIN
							    SELECT  @TotalEntry = COUNT(DISTINCT AM.ReportId)
				  FROM    dbo.View_SeenClientAnswerMaster AS AM
                                        INNER JOIN dbo.SeenClientAnswers AS A ON AM.ReportId = A.SeenClientAnswerMasterId
 ...
 /*               FROM    dbo.View_SeenClientAnswerMaster AS AM
          ...
          --                                              )
			...
			DECLARE @OptionCount BIGINT
			DECLARE @OptionCountBenchmark BIGINT
			PRINT 'Yours'
			IF EXISTS (SELECT 1 FROM @Result)
			BEGIN
			SELECT @OptionCount = SUM(Score),@OptionCountBenchmark = SUM(BenchmarkScore) FROM @Result
			END
            ELSE
			BEGIN
			    SET @OptionCount = 1
				SET @OptionCountBenchmark = 1
			END
			PRINT 'Yours1'
			PRINT @OptionCountBenchmark
				SELECT  Tc.OptionId ,
                Tc.Name ,
                ...
                CASE WHEN @QuestionTypeId = 19
                     THEN ( CASE WHEN ISNULL(SUM(R.Score), 0)
                                 ...
                                 THEN ISNULL(SUM(R.Score), 0) * 100.00
                                       / CASE ISNULL(SUM(R.Score), 0) WHEN 0 THEN 1 ELSE ISNULL(SUM(R.Score), 0) end
                                 ELSE 0
                            END )
                     ...
                     THEN ( CASE WHEN ISNULL(SUM(R.Score), 0)
                                      + ISNULL(SUM(R.BenchmarkScore), 0) > 0
                                 THEN ISNULL(SUM(R.Score), 0) * 100.00
                                      / CASE ISNULL(@OptionCount,0) WHEN 0 THEN 1 ELSE @OptionCount end
                                 ELSE 0
                            END )
                     ELSE CAST(ISNULL(SUM(R.Score), 0)
                          / CASE ISNULL(@OptionCount, 0)
                              WHEN 0 THEN 1
                              ELSE ISNULL(@OptionCount, 0)
                            END * 100.0 AS DECIMAL(18, 2))
                END AS YourPerformance ,
                CASE WHEN (@SellectAllUser = -1 AND @SellectAllEstablishment = -1) THEN -1 ELSE ISNULL(SUM(R.BenchmarkScore), 0) end AS BenchmarkScore ,
                CASE WHEN @QuestionTypeId = 19
                     THEN ( CASE WHEN ISNULL(SUM(R.Score), 0)
                                      + ISNULL(SUM(R.BenchmarkScore), 0) > 0
                                 THEN ISNULL(SUM(R.BenchmarkScore), 0)
                                      * 100.00 / CASE ISNULL(@OptionCountBenchmark,0) WHEN '' THEN 1 ELSE @OptionCountBenchmark end
                                 ELSE 0
                            END )
                     WHEN @QuestionTypeId = 7
                          OR @QuestionTypeId = 14
                     ...
                     THEN ( CASE WHEN ISNULL(SUM(R.Score), 0)
                                      + ISNULL(SUM(R.BenchmarkScore), 0) > 0
                                 THEN ISNULL(SUM(R.BenchmarkScore), 0)
                                      * 100.00 / CASE ISNULL(@OptionCountBenchmark,0) WHEN 0 THEN 1 ELSE @OptionCountBenchmark end
                            END )
                     ELSE CAST(ISNULL(SUM(R.BenchmarkScore), 0)
                          / CASE ISNULL(@OptionCountBenchmark, 0)
                              WHEN 0 THEN 1
                              ELSE ISNULL(@OptionCountBenchmark, 0)
                            END * 100.0 AS DECIMAL(18, 2))
                END AS BenchmarkPerformance ,
                @LocalTime AS LastUpdatedTime ,
                @FromDate AS StartDate ,
                @EndDate AS EndDate ,
                ISNULL(CAST(@TodayEntry AS INT), 0) AS TotalEntry
        FROM    @tblCount AS Tc
