CREATE PROCEDURE dbo.GroupCopy_202020707
    @UserId BIGINT,
    @GroupId BIGINT,
    @Activity BIT,
    @Establishment BIT,
...
and c1.SeenClientId = ' + CONVERT(NVARCHAR(200), @vNewSeenClientId) + '
 and c.Id in (' + @vOldmobiFormDisplayFields + ')FOR XML PATH('''')) ,1,1,'''')';
    ...
    SELECT DISTINCT a.QuetionId,
           ...
           0,
           a.IsRoutingOnGroup,
           a.ConditionRepetitiveGroupId
    FROM
    (
        SELECT ROW_NUMBER() OVER (PARTITION BY q.NewQueId ORDER BY cl.Id DESC) RNum,
               ROW_NUMBER() OVER (PARTITION BY qq.NewQueId ORDER BY cl.Id DESC) RNum_New,
